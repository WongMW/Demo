        ...
        Private Sub LoadMembers(ByVal lChapterID As Long, ByVal bLoadChapterMember As Boolean)
            ...
            Try
                If ViewState(ATTRIBUTE_DATATABLE_CHAPTER_MEMBER) IsNot Nothing Then
                    grdMembers.DataSource = ViewState(ATTRIBUTE_DATATABLE_CHAPTER_MEMBER)
                Else
                    sSQL = Database & _
                    ...
                    grdMembers.DataBind()
                    If bLoadChapterMember Then
                        If dt.Rows.Count > 0 Then
                            'grdMembers.SelectedIndex = 0
                            'CP 2/2/09 ISSUE 8214. Remove chapter edit control.
                            'LoadChapterMember()
                            'ChapterMember1.Visible = True
                        Else
                            'Hide the chapter member dialog if there are no 
                            'records to display.  This is very confusing b/c that
                            'dialog is not intended to create a new member, only
                            'to edit existing member info.  RN 3/22/2003.
                            'CP 2/2/09 ISSUE 8214. Remove chapter edit control.
                            'ChapterMember1.Visible = False
                        End If
                    End If
                End If
            Catch ex As Exception
