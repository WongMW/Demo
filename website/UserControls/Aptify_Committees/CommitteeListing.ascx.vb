        ...
        Private Sub LoadCommittees()
            ...
            Try
                If ViewState(ATTRIBUTE_DATATABLE_COMMITTEE) IsNot Nothing Then
                    grdCommittees.DataSource = ViewState(ATTRIBUTE_DATATABLE_COMMITTEE)
                Else
                    ...
                    If Not Me.SetControlRecordIDFromParam() Then
                        sSQL = "Select ID,Name,Description,Goals,DateFounded FROM " & _
                               sDatabase & _
                               "..vwCommittees WHERE IsActive = 1 AND CommitteeType <> 'Network' ORDER BY Name"
                    Else
                        sSQL = "SELECT ID,Name,Description,Goals,DateFounded FROM " & _
                               sDatabase & _
                               "..vwCommittees WHERE  CommitteeType <> 'Network' AND ParentID=" & Me.ControlRecordID.ToString & " ORDER BY Name"
                    End If
