...
'-----------------------------------------------------------------------------------------------------------------------------------------------------
'Govind Mande               9 Sept 2015                     Update make mentor on person information and changes  inline query to sp and Messge from culture string
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Namespace Aptify.Framework.Web.eBusiness.CustomerService
    Partial Class AdminEditProfile__c
        ...
        Protected Sub btnOk_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnOk.Click
            If txtEditFirstName.Text = "" Or txtEditLastName.Text = "" Then
                radwindowPopup.VisibleOnPageLoad = True
            Else
                ' Govind Mande - check person if non member then give popup 
                Dim sSql As String = Database & "..spGetPersonIsMember__c @MemberTypeID=" & ViewState("MemberTypeID")
                Dim IsMember As Boolean = Convert.ToBoolean(DataAction.ExecuteScalar(sSql, IAptifyDataAction.DSLCacheSetting.BypassCache))
                If IsMember = False Then
                    If chkMakeMentor.Checked = True Then
                        ' open popup 
                        If lblMakeMentor.Text.Trim.ToLower = "no" Then
                            radwindowPopup.VisibleOnPageLoad = False
                            lblNonMemberMsg.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.AdminEditProfile__c.NonMemberCreateMakeMentor")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                            radNonMemberMsg.VisibleOnPageLoad = True
                        Else
                            DoSave()
                            radwindowPopup.VisibleOnPageLoad = False
                        End If
...
#End Region
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not IsPostBack Then
                ...
                If Session("PersonID") IsNot Nothing AndAlso IsNumeric(Session("PersonID")) AndAlso CLng(Session("PersonID")) <> -1 Then
                    LoadForm()
                    'Amruta 14307 start
        ...
        Private Sub LoadAddresses()
            Try
                If dt.Rows.Item(0) IsNot Nothing Then
                    ...
                    SetComboValue(cmbCountry, IIf(dt.Rows.Item(0)("BusinessCountry").ToString() = "", "Ireland", dt.Rows.Item(0)("BusinessCountry")).ToString)
                    PopulateState(cmbState, cmbCountry)
                    ...
                    SetComboValue(cmbHomeCountry, IIf(dt.Rows.Item(0)("HomeCountry").ToString() = "", "Ireland", dt.Rows.Item(0)("HomeCountry")).ToString)
                    PopulateState(cmbHomeState, cmbHomeCountry)
                    ...
                    SetComboValue(cmbBillingCountry, IIf(dt.Rows.Item(0)("BillingCountry").ToString() = "", "Ireland", dt.Rows.Item(0)("BillingCountry")).ToString)
                    PopulateState(cmbBillingState, cmbBillingCountry)
                    ...
                    SetComboValue(cmbPOBoxCountry, IIf(dt.Rows.Item(0)("POBoxCountry").ToString() = "", "Ireland", dt.Rows.Item(0)("POBoxCountry")).ToString)
                    PopulateState(cmbPOBoxState, cmbPOBoxCountry)
        ...
        Private Sub LoadInformation()
            ...
            Try
                ' Govind Mande change inline query to SP
                sSQL = Database & "..spGetPersonEditProfileForAdmin__c @PersonID=" & CType(Session.Item("PersonID"), Long)
                ...
                ' Below code added by Govind Mande
                'Added by Kavita ZInage #18296
                If Convert.ToBoolean(dt.Rows(0)("IsMentor__c")) Then
                    lblMakeMentor.Text = "Yes"
                    chkMakeMentor.Enabled = False 'Added by Kavita ZInage #18296
                Else
                    lblMakeMentor.Text = "No"
                    chkMakeMentor.Enabled = True 'Added by Kavita ZInage #18296
                End If
                'Added by Kavita ZInage #18296
                If String.Compare(Convert.ToString(dt.Rows(0)("MemberType")), "Student", True) = 0 Then
                    chkMakeMentor.Enabled = False
                End If
                'Till Here

                chkMakeMentor.Checked = Convert.ToBoolean(dt.Rows(0)("IsMentor__c"))
                ViewState("MemberTypeID") = Convert.ToInt32(dt.Rows.Item(0)("MemberTypeID").ToString())
        ...
        Private Function DoSave() As Boolean
            Try
                ...
                With oGE
                    ...
                    ' Below code added by Govind Mande for save Make Mentor Status
                    If ViewState("MakeMentorNo") = "No" Then
                    Else
                        oGE.SetValue("IsMentor__c", chkMakeMentor.Checked)
...
#End Region
        ...
        Private Sub LoadProfilePicture(ByVal ImageName As String)
            ...
            ' Govind Mande change inline query to SP
            sSQL = Database & "..spGetPersonPhoto__c @PersonID=" & m_lRecordID
        ...
        Protected Sub btnsend_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnsend.Click
            Try
                ...
                ' Govind Mande change inline query to SP
                Dim sSql As String = Database & "..spGetProcessFlowIDFromName__c @Name='Send Email Notification for Profile Edits'"
                ...
                If oProcessFlowID IsNot Nothing AndAlso IsNumeric(oProcessFlowID) Then
                    ...
                    If Not oResult.IsSuccess Then
                        ...
                        lblmsg.Visible = False
                    Else
                        lblmsg.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.AdminEditProfile__c.NotificationSentMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                        lblmsg.Visible = True
                    End If
        ...
        Public Overridable Function LoadTopicCodesParent(ByVal sEntityName As String, ByVal sRecordID As String) As String
            ...
            ' Govind Mande change inline query to SP
            Dim sSql As String = Database & "..spGetLoadTopicCodeDetails__c @RecordID=" & sRecordID & ",@EntityName='" & sEntityName & "'"
            dt = DataAction.GetDataTable(sSQL)
        ...
        Private Function IsCompanyPerson(ByVal PersonID As Long) As Boolean
            Try
                ...
                ' Govind Mande change inline query to SP
                sSql = Database & "..spCompanyIDForPerson @PersonID=" & PersonID
        ...
        End Function
        ''' <summary>
        ''' Govind Mande added below code for update Make Mentor Value True
        ''' </summary>
        ''' <param name="sender"></param>
        ''' <param name="e"></param>
        ''' <remarks></remarks>
        Protected Sub btnMakeMentorYes_Click(sender As Object, e As System.EventArgs) Handles btnMakeMentorYes.Click
            Try
                radwindowPopup.VisibleOnPageLoad = False
                radNonMemberMsg.VisibleOnPageLoad = False
                ViewState("MakeMentorNo") = "Yes"
                DoSave()
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
            End Try
        End Sub
        ''' <summary>
        ''' Govind Mande added below code for update Make Mentor Value False
        ''' </summary>
        ''' <param name="sender"></param>
        ''' <param name="e"></param>
        ''' <remarks></remarks>
        Protected Sub btnMakeMentorNo_Click(sender As Object, e As System.EventArgs) Handles btnMakeMentorNo.Click
            Try
                radwindowPopup.VisibleOnPageLoad = False
                ViewState("MakeMentorNo") = "No"
                radNonMemberMsg.VisibleOnPageLoad = False
                DoSave()
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
