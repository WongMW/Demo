...
Namespace Aptify.Framework.Web.eBusiness.Education
    Partial Class ClassRegistration__c
...
#End Region
        ...
        Protected Sub btnSaveRegistration_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveRegistration.Click, btnSaveRegistrationPaid.Click
            Try
                ...
                Dim lProductID As Long = CLng(lblProductID.Value)
                If lProductID > 0 Then
                    If ValidateTermsAndCondtionForIndividual(lProductID, True) = False Then
                        Exit Sub
                    End If
                End If
                ...
                Else
                    ...
                    If ValidateRegistrants() Then
                        ...
                        'added by asmita
                        ValidateTermsAndCondtionForIndividual(lProductID, False)
                        '
        ...
        Private Sub SetupGrid()
            Try
                ...
                '**Hooman, Issue 8840: removed order object clearing causing shopping cart to emtpy upon class registration
                'oOrderGE = Me.ShoppingCart1.GetOrderObject(Session, Page.User, Application)
                'If Not IsNothing(oOrderGE) Then oOrderGE.SubTypes("OrderLines").Clear()
                '** end Issue 8840

                'Dim oClassRegistrationGE As Aptify.Framework.BusinessLogic.GenericEntity.AptifyGenericEntityBase
                'Dim oPersonsGE As Aptify.Framework.BusinessLogic.GenericEntity.AptifyGenericEntityBase

                'For Each OrderLine As Aptify.Applications.OrderEntry.OrderLinesEntity In oOrderGE.SubTypes("OrderLines")
                '    If Not IsNothing(OrderLine.ExtendedOrderDetailEntity) AndAlso OrderLine.ExtendedOrderDetailEntityName.ToUpper = "CLASS REGISTRATIONS" Then
                '        oClassRegistrationGE = OrderLine.ExtendedOrderDetailEntity
                '        If oClassRegistrationGE.GetValue("ClassID").ToString = Request.QueryString("ClassID").ToString And _
                '        CLng(oClassRegistrationGE.GetValue("StudentID").ToString) <> User1.PersonID Then
                '            oPersonsGE = Me.AptifyApplication.GetEntityObject("Persons", CLng(oClassRegistrationGE.GetValue("StudentID").ToString))
                '            dt.Rows.Add(dt.NewRow)
                '            dt.Rows(dt.Rows.Count - 1).Item(0) = oPersonsGE.RecordID
                '            dt.Rows(dt.Rows.Count - 1).Item(1) = oPersonsGE.GetValue("FirstName").ToString
                '            dt.Rows(dt.Rows.Count - 1).Item(2) = oPersonsGE.GetValue("LastName").ToString
                '            dt.Rows(dt.Rows.Count - 1).Item(3) = oPersonsGE.GetValue("Title").ToString
                '            dt.Rows(dt.Rows.Count - 1).Item(4) = oPersonsGE.GetValue("EMail").ToString
                '        Else
                '            oOrderGE.SubTypes("OrderLines").Remove(OrderLine)
                '        End If
                '    End If
                'Next
                Me.ShoppingCart1.SaveCart(Me.Session)
            ...
            Try
                ...
                If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                    With dt.Rows(0)
                        ...
                        'added asmita 1/13/2014
                        If Not String.IsNullOrEmpty(lblProductID.Value) AndAlso IsNumeric(lblProductID.Value) AndAlso CLng(lblProductID.Value) > 0 Then
                            LoadTermsAndCondtion(CLng(lblProductID.Value))
                        End If
                        '
                    End With
...
#Region "Custom Subroutine/Methods"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function ValidateTermsAndCondtionForIndividual(ByVal lProductID As Long, ByVal IsValidation As Boolean) As Boolean
            Dim dttermandconditions As DataTable = Nothing
            Dim sSQL As String = String.Empty, bRetVal As Boolean = False
            Try
                ...
                If dttermandconditions IsNot Nothing AndAlso dttermandconditions.Rows.Count > 0 AndAlso CBool(dttermandconditions.Rows(0)("IsRequiredAgreement__c")) = True Then
                    ...
                    Else
                        ...
                        If IsValidation = False Then
                            ...
                            oOrderLine = CType(oOrder.SubTypes("OrderLines").Find("ProductID", lProductID), Aptify.Applications.OrderEntry.OrderLinesEntity)
                    ...
                    End If
                Else
                    If Request.QueryString("req") IsNot Nothing AndAlso Not IsNumeric(Request.QueryString("req")) & _
                    Request.QueryString("term") IsNot Nothing AndAlso Not IsNumeric(Request.QueryString("term")) Then

                        Dim oOrder As Aptify.Applications.OrderEntry.OrdersEntity = Nothing
                        Dim oOrderLine As Aptify.Applications.OrderEntry.OrderLinesEntity = Nothing
                        oOrder = ShoppingCart1.GetOrderObject(Page.Session, Page.User, Page.Application)
                        oOrderLine = CType(oOrder.SubTypes("OrderLines").Find("ProductID", lProductID), Aptify.Applications.OrderEntry.OrderLinesEntity)
                        oOrderLine.SetValue("IsRequiredAgreement__c", CBool(Request.QueryString("req")))
                        oOrderLine.SetValue("TermsAndConditionID__c", CLng(Request.QueryString("term")))
                        ShoppingCart1.SaveCart()
                        bRetVal = True
                    Else
                        bRetVal = False
        ...
        ''' <remarks></remarks>
        Private Sub LoadTermsAndCondtion(ByVal lProductID As Long)
