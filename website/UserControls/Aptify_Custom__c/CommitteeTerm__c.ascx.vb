...
'-----------------------------------------------------------------------------------------------------------------------------------------------------
'Pratiksha Bankar              16/04/2014                    Display the pref. mobile number and Is council member checkbox while displaying the member of the committee.
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Namespace Aptify.Framework.Web.eBusiness.Custom
    Partial Class CommitteeTermControl__c
        ...
        Protected Const ATTRIBUTE_CommitteeTermMeetingPageURL As String = "CommitteeTermMeetingPage"
        Protected Const ATTRIBUTE_Meeting_IMAGE_SMALL As String = "MeetingImage"
...
#Region "CommitteeTerm Specific Properties"
        ...
        ''' GeneralImageSmall url
        ''' </summary>
        Public Overridable Property MeetingImageSmall() As String
            Get
                If Not ViewState(ATTRIBUTE_Meeting_IMAGE_SMALL) Is Nothing Then
                    Return CStr(ViewState(ATTRIBUTE_Meeting_IMAGE_SMALL))
                Else
                    Return String.Empty
                End If
            End Get
            Set(ByVal value As String)
                ViewState(ATTRIBUTE_Meeting_IMAGE_SMALL) = Me.FixLinkForVirtualPath(value)
            End Set
        End Property

        ''' <summary>
        ''' CommitteeTermMeetingPage url
        ''' </summary>
        Public Overridable Property CommitteeTermMeetingPage() As String
            Get
                If Not ViewState(ATTRIBUTE_CommitteeTermMeetingPageURL) Is Nothing Then
                    Return CStr(ViewState(ATTRIBUTE_CommitteeTermMeetingPageURL))
                Else
                    Return String.Empty
                End If
            End Get
            Set(ByVal value As String)
                ViewState(ATTRIBUTE_CommitteeTermMeetingPageURL) = Me.FixLinkForVirtualPath(value)
            End Set
        End Property
#End Region
        ...
        Protected Overrides Sub SetProperties()
            ...
            If String.IsNullOrEmpty(MeetingImageSmall) Then
                MeetingImageSmall = Me.GetLinkValueFromXML(ATTRIBUTE_Meeting_IMAGE_SMALL)
            End If
            If String.IsNullOrEmpty(CommitteeTermMeetingPage) Then
                Me.CommitteeTermMeetingPage = Me.GetLinkValueFromXML(ATTRIBUTE_CommitteeTermMeetingPageURL)
            End If
            'code End by Govind
        End Sub
        ...
        Private Sub SetLeftNavImageLinks()
            ...
            MeetingSmall.Src = MeetingImageSmall
        End Sub
        ...
        Private Sub SetupView(ByVal CommitteeTermID As Long)
            Try
                ...
                Else
                    ...
                    trTermMeeting.Visible = False
                    Response.Redirect(Me.GetSecurityErrorPageFromXML & "?Message=" & Server.UrlEncode("Committee Term not available"), True)
                End If

                LoadGeneral(CommitteeTermID) ' always load general first b/c other tabs may use this information at times.
                ViewState("CommitteeTermID") = CommitteeTermID
                Select Case m_sView
                    Case "Forum"
        ...
        Private Sub LoadMembers(ByVal CommitteeTermID As Long)
            ...
            Try
                ...
                Else
                    ...
                    ' SP created by Pratiksha on 16/04/2014
                    sSQL = Database & "..spDisplayCommitteeMember__c @CommitteeTermID=" & CommitteeTermID
                    dt = DataAction.GetDataTable(sSQL, IAptifyDataAction.DSLCacheSetting.BypassCache)
        ...
        End Sub
        'Code Added by Govind
        Protected Sub lnkMeeting_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkMeeting.Click
            Try
                Response.Redirect(CommitteeTermMeetingPage & "?TermID=" & Convert.ToString(ViewState("CommitteeTermID")))
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
            End Try
        End Sub
        ''' <summary>
        ''' If Member In Active then hide Meeting Link
        ''' </summary>
        ''' <remarks></remarks>
        Private Sub CheckCommitteeTermMemberInActive()
            Try
                Dim sSql As String = Database & "..spCheckCommitteeMemberInActive__c @PersonID=" & User1.PersonID & ",@TermID=" & Convert.ToString(ViewState("CommitteeTermID"))
                Dim lNonMemberID As Long = Convert.ToInt64(DataAction.ExecuteScalar(sSql, IAptifyDataAction.DSLCacheSetting.BypassCache))
                If lNonMemberID > 0 Then
                    trTermMeeting.Visible = False
                End If
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
