...
Namespace Aptify.Framework.Web.eBusiness.ProductCatalog
    Partial Class ExpoRegistration__c
        ...
        Private Sub cmdSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdSave.Click
            Try
                ...
                If ValidateOrderBoothDetail() Then
                    ...
                    'Anil Issue 12660
                    OrderGE = ShoppingCart1.GetOrderObject(Page.Session, Page.User, Page.Application)
                    oOrderLine = CType(OrderGE.SubTypes("OrderLines").Item(iLine), OrderLinesEntity)
                    lProductID = CLng(oOrderLine.GetValue("ProductID"))
                    'End

                    'added by asmita on 1/10/2014, 
                    FlowDownTermsAndCondtions(CType(oOrder, OrdersEntity), lProductID, oOrderLine)
                    '

                    oExpoDetail = AptifyApplication.GetEntityObject("OrderBoothDetail", -1)
                    sData = CStr(oOrder.SubTypes("OrderLines").Item(iLine).GetValue("_x_ExtendedAttributeObjectData"))
                    If Len(sData) > 0 Then
                    ...
                    Else
                        ...
                        If Session("ProductID") IsNot Nothing AndAlso CLng(Session("ProductID")) > -1 Then
                            For iItem = 0 To oOrder.SubTypes("OrderLines").Count - 1
                                If CLng(oOrder.SubTypes("OrderLines").Item(iItem).GetValue("ProductID")) = CLng(Session("ProductID")) AndAlso _
                                   CLng(oOrder.SubTypes("OrderLines").Item(iItem).GetValue("ParentSequence")) <= 0 Then
                                    oOrderLine = CType(oOrder.SubTypes("OrderLines").Item(iItem), OrderLinesEntity)
                    ...
                    Response.Redirect(SaveButtonRedirect)
                End If
...
#Region "Custom Methods/Functions"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Sub FlowDownTermsAndCondtions(ByVal oOrder As Aptify.Applications.OrderEntry.OrdersEntity, ByVal lProductID As Long, ByVal oOrderLine As Aptify.Applications.OrderEntry.OrderLinesEntity)
            Try
                Dim sSQL = "SELECT TermsAndConditionID__c FROM " & _
                    AptifyApplication.GetEntityBaseDatabase("Products") & _
                     "..vwProducts WHERE IsRequiredAgreement__c =1 and ID=" & lProductID

                Dim obj As Object = DataAction.ExecuteScalar(sSQL, DataServices.IAptifyDataAction.DSLCacheSetting.BypassCache)
                If Not obj Is Nothing AndAlso System.Convert.ToInt64(obj) > 0 Then
