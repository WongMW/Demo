...
'Rajesh Kardile             03/20/2014                          Change inline Query to Store procedure SpGetProductForMultipleCategory__c
...
Namespace Aptify.Framework.Web.eBusiness.ProductCatalog
    Partial Class FindProductControl__c
        ...
        Private _clickPosition As Int32 = 0
...
#End Region
        ...
        Private Sub cmdSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSearch.Click
            ...
            '                lnk = CType(row.FindControl("lnkCategory"), HyperLink)
            '                lnk.NavigateUrl = String.Format(ViewProductCatagoryPage & "?ID={0}", dt.Rows(row.RowIndex)("CategoryID").ToString)
            '            Next
        ...
        Private Sub ShowResultTable()
            ...
            Try
                ...
                sName = Trim$(txtName.Text)
                sDescription = Trim$(txtDescription.Text)
                If Len(sName) > 0 Or Len(sDescription) > 0 Then
                    'Rajesh K - 03/20/2014
                    '************************** Start ************************************
                    ...
                    If User1.UserID > 0 Then
                        ...
                        End If
                    End If
                    Dim sProdctsList As String = Database & "..SpGetProductForMultipleCategory__c"
                    Dim params(4) As System.Data.IDataParameter
                    params(0) = DataAction.GetDataParameter("@Name", SqlDbType.NVarChar, sName)
                    params(1) = DataAction.GetDataParameter("@Description", SqlDbType.NVarChar, sDescription)
                    params(2) = DataAction.GetDataParameter("@CategoryID", SqlDbType.Int, 0)
                    params(3) = DataAction.GetDataParameter("@ShowMeetingsLinkToClass", SqlDbType.Bit, ShowMeetingsLinkToClass)
                    params(4) = DataAction.GetDataParameter("@CurrencyType", SqlDbType.VarChar, CurrencyType)
                    ...
                    If dt.Rows.Count > 0 Then
                        ...
                        'For Each row As GridViewRow In grdResults.Rows
                        '    Dim lnk As HyperLink = CType(row.FindControl("lnkWebName"), HyperLink)
                        '    lnk.NavigateUrl = String.Format(ViewProductPage & "?ID={0}", dt.Rows((grdResults.PageIndex * grdResults.PageSize) + rowcounter)("ID").ToString)
                        '    lnk = CType(row.FindControl("lnkCategory"), HyperLink)
                        '    lnk.NavigateUrl = String.Format(ViewProductCatagoryPage & "?ID={0}", dt.Rows((grdResults.PageIndex * grdResults.PageSize) + rowcounter)("CategoryID").ToString)
        ...
        Protected Function GetGtmObject(dataItem As Object) As String
            ...
            Return "trackGTMClick('" +
                   ...
                   dtRow("Category").ToString + "','" +
                   "" + "','" +
                   _clickPosition.ToString + "','" +
                   dtRow("ViewProductPageUrl").ToString + "','" +
                   currencyType + "'" +
                   ")"
