...
'-----------------------------------------------------------------------------------------------------------------------------------------------------
'Govind Mande               9 Sept 2015                     Add people to subsidary company functionality
'-----------------------------------------------------------------------------------------------------------------------------------------------------
Option Explicit On
...
Namespace Aptify.Framework.Web.eBusiness.CustomerService
    Partial Class GAAddMember__c
        ...
        Protected Const ATTRIBUTE_CompanyDirectory_PAGE As String = "CompanyDirectory"
        Enum GridColIndex
            RowNumber = 0
            FirstName = 1
            LastName
        ...
        Public Overridable Property LoginPage() As String
            ...
            End Set
        End Property
        ' ATTRIBUTE_CompanyDirectory_PAGE
        Public Overridable Property CompanyDirectory() As String
            Get
                If Not ViewState(ATTRIBUTE_CompanyDirectory_PAGE) Is Nothing Then
                    Return CStr(ViewState(ATTRIBUTE_CompanyDirectory_PAGE))
                Else
                    Return String.Empty
                End If
            End Get
            Set(ByVal value As String)
                ViewState(ATTRIBUTE_CompanyDirectory_PAGE) = Me.FixLinkForVirtualPath(value)
            End Set
        End Property
        ...
        Protected Overrides Sub SetProperties()
            ...
            If String.IsNullOrEmpty(CompanyDirectory) Then
                'since value is the 'default' check the XML file for possible custom setting
                CompanyDirectory = Me.GetLinkValueFromXML(ATTRIBUTE_CompanyDirectory_PAGE)
            End If
        End Sub
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not Request.QueryString("cID") Is Nothing Then
                ViewState("CompanyID") = Convert.ToInt32(Aptify.Framework.Web.Common.WebCryptography.Decrypt(Request.QueryString("cID")))
                cmbSubsidarisCompney.Visible = False
                btnBack.Visible = True
            End If
            If ViewState("CompanyID") Is Nothing Then
                ViewState("CompanyID") = User1.CompanyID
                cmbSubsidarisCompney.Visible = True
                btnBack.Visible = False
            End If
            ' code end by Govind Mande
        ...
        End Sub
        ''' <summary>
        ''' Govind Mande add below function for get all child company 
        ''' </summary>
        ''' <remarks></remarks>
        Private Sub LoadSubsidiariesCompanyList()
            Try

                Dim sSql As String = Database & "..spGetAllSubsidiariesWithCityName__c @ParentCompanyId=" & User1.CompanyID
        ...
        End Sub
        ''' <summary>
        ''' Govind Mande add below code for selected company id store and set intial row value
        ''' </summary>
        ...
        Protected Sub cmbSubsidarisCompney_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles cmbSubsidarisCompney.SelectedIndexChanged
            Try
                ViewState("CompanyID") = cmbSubsidarisCompney.SelectedValue
                SetInitialRow()
                drpRows.SelectedValue = "1"
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
            End Try
        End Sub
        Private Sub SetInitialRow()
            ...
            dr("RowNumber") = 0 '1 Changed by Shital from 1 to 0, on 30 Oct 2015, IssueID: 4880- last row was not deleting.
            dr("First Name") = String.Empty
            dr("Last Name") = String.Empty
            dr("Title") = String.Empty
            dr("Email") = String.Empty
            'dr("Company") = User1.Company
            dr("Company") = Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID")))
            ...
            If Session("CurrentTable") IsNot Nothing Then
                ...
                If dtCurrentTable.Rows.Count > 0 Then
                    For i As Integer = 1 To dtCurrentTable.Rows.Count
                        ...
                        drCurrentRow("RowNumber") = i '+ 1 Commented by Shital on 30 Oct 2015, IssueID: 4880- last row was not deleting.
                        ...
                        dtCurrentTable.Rows(i - 1)("Company") = Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID")))
            ...
            If Session("CurrentTable") IsNot Nothing Then
                ...
                If dt.Rows.Count > 0 Then
                    For i As Integer = 0 To dt.Rows.Count - 1
                        ...
                        lblGACompany.Text = Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID")))
            ...
            If Session("FailedDataTable") IsNot Nothing Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    If dt.Rows.Count > 0 And dt.Rows.Count = itotItems AndAlso InsertedData = 0 Then
                        ...
                        'End If
                        lblResult.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__c.EmailExistsMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                    ElseIf dt.Rows.Count = 0 Then
                        lblResult.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__c.SuccessfullMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
        ...
        Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As EventArgs)
            ...
            If Not ValidateEnteredData() Then
                lblResult.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__cInvalidInfoMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
            Else
                ...
                Else
                    ...
                    Else
                        lblResult.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__cValidDataMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                    End If
                End If
                getGridData()
            End If
            ...
            Try
                If (txtFname.Text.Trim <> "" AndAlso txtLName.Text.Trim <> "" AndAlso txtEmail.Text.Trim <> "") Then
                        ...
                        If RecordID > -1 Then
                            ...
                            If lNewRecCountOfPersons > lPrevRecCountOfPersons Then
                                ...
                                If oPersonGE IsNot Nothing Then
                                    ...
                                    oPersonGE.SetValue("CompanyID", ViewState("CompanyID"))
                                    oPersonGE.SetValue("Status__c", "Pending") ' code added by govind mande
                            ...
                            Else
                                ...
                                dtFailedData = AddFailedDataTodt(dtFailedData, txtFname.Text.Trim, txtLName.Text.Trim, txtTitle.Text.Trim, txtEmail.Text.Trim, Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID"))), False)
                            End If
                        Else
                            ''TODO: if failed to add record add it to data table
                            dtFailedData = AddFailedDataTodt(dtFailedData, txtFname.Text.Trim, txtLName.Text.Trim, txtTitle.Text.Trim, txtEmail.Text.Trim, Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID"))), False)
                    ...
                    ElseIf chkCreateWebUser.Checked = True Then ' if web user to be created
                        ...
                        user.SetValue("Company", Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID"))))
                        user.SetValue("WebUserStringID", txtEmail.Text.Trim)
                        user.SetValue("Password", GetRandomPasswordUsingGUID(7))
                        ...
                        If user.SaveUser(Page.Session, False) Then
                            ...
                            If oPersonGE IsNot Nothing Then
                                oPersonGE.SetValue("Title", txtTitle.Text.Trim)
                                oPersonGE.SetValue("CompanyID", ViewState("CompanyID")) 'code added by govind mande
                                oPersonGE.SetValue("Status__c", "Pending") 'code added by govind mande
                                If oPersonGE.Save(False) Then
                                End If
                            End If
                            sSQL = Database & "..spGetUserIDByLinkID__c @LinkID=" & Convert.ToString(user.PersonID)
                            lWebUserID = Convert.ToInt64(DataAction.ExecuteScalar(sSQL))
                            SendWebCredentialsMail(user.PersonID, lWebUserID)
                        Else
                            ''TODO: if failed to add record add it to data table
                            dtFailedData = AddFailedDataTodt(dtFailedData, txtFname.Text.Trim, txtLName.Text.Trim, txtTitle.Text.Trim, txtEmail.Text.Trim, Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID"))), True)
                        End If
                    End If
                Else
        ...
        Private Function GetMaxUserID() As Long
            Try
                Dim sSQL As String
                sSQL = Database & "..spGetLatestPersonID__c"
        ...
        Public Function AddFailedDataTodt(ByRef dtFailedRowTable As DataTable, ByVal sFName As String, ByVal sLName As String, ByVal sTitle As String, ByVal sEmail As String, ByVal sCompnay As String, ByVal bWebUser As Boolean) As DataTable
            ...
            drFailedRow("Company") = Convert.ToString(AptifyApplication.GetEntityRecordName("Companies", ViewState("CompanyID")))
        ...
        Protected Sub SendWebCredentialsMail(ByVal lPersonID As Long, ByVal lWebUserID As Long)
            Try
                ...
                Dim sSql As String = Database & "..spGetProcessFlowIDFromName__c @Name='GA_AddMembers'"
                ...
                If (xlsUpload.UploadedFiles IsNot Nothing AndAlso xlsUpload.UploadedFiles.Count <> 0) Then
                    ...
                    If sFileUpload.Trim().ToLower() = ".xls" OrElse sFileUpload.Trim().ToLower() = ".xlsx" Then
                        ...
                        Catch ex As FileNotFoundException
                            Me.lblMessage.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__UploadCorrectFileMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                        ...
                        Catch generatedExceptionName As Exception
                            uploadFlag = False
                            Me.lblMessage.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__UploadingErrorMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                            lblUploadedExcel.Visible = False
                            ExportExcel.Visible = False
                            radImgSmallExcel.Visible = False
                        End Try
                    ...
                    If uploadFlag = True Then
                        Me.lblMessage.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__SuccessfullyMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                            ...
                            For Each row As DataRow In dtvalidation.Rows
                                    ...
                                    If CommonMethods.EmailAddressCheck(Convert.ToString(row("Email")).Trim) Then
                                        ...
                                        If Convert.ToInt16(Convert.ToString(row("Create Web User?")).Trim) = 0 Then
                                            ...
                                            If RecordID > -1 Then
                                                ...
                                                If lSecondPersonRecordID <> -1 AndAlso lFirstPersonRecordID = lSecondPersonRecordID Then
                                                    row("Remark") = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__cRecordFailedToUploadMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                                                Else
                                                    ...
                                                    If oPersonGE IsNot Nothing Then
                                                        ...
                                                        If oPersonGE.Save(False) Then
                                                            row("Remark") = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__cRecordCreatedSuccfullyMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                                                        Else
                                                            row("Remark") = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__cRecordFailedTryMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                                                        End If
                                                        'Else
                                        ...
                                        ElseIf Convert.ToInt16(Convert.ToString(row("Create Web User?")).Trim) = 1 Then ' if web user to be created
                                            ...
                                            If user.SaveUser(Page.Session, False) Then
                                                ...
                                                sSQL = Database & "..spGetUserIDByLinkID__c @LinkID=" & Convert.ToString(user.PersonID).Trim
                                                lWebUserID = Convert.ToInt64(DataAction.ExecuteScalar(sSQL))
                                                SendWebCredentialsMail(user.PersonID, lWebUserID)
                                            Else
                                                row("Remark") = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__cRecordFailedToUploadMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                                ...
                                Else
                                    ...
                                    ElseIf Convert.ToInt32(row("Create Web User?")) <> 1 Then
                                        row("Create Web User?") = 0
                                    End If
                                    row("Remark") = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.GAAddNewMember__cRecordFailedToUploaIncompleteInfolMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                                End If
                            Next
                        End If
                    End If
        ...
        Protected Sub btnBack_Click(sender As Object, e As System.EventArgs) Handles btnBack.Click
            Try
                Response.Redirect(CompanyDirectory, False)
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
