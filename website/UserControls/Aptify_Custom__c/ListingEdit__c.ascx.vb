...
'Kavita Zinage        07/22/2015                      Marketplace ListingEdit Page
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Option Explicit On
Option Strict On
    ...
    Partial Class ListingEdit
        ...
        Public Function SetNewListing() As Boolean
            ...
            Try
                ...
                chkAti.Checked = False
                chkGraduate.Checked = False
                chkSchoolLeaver.Checked = False
                SetNewListing = True
        ...
        Public Function LoadListing(ByVal ListingID As Long) As Boolean
            ...
            Try
                ...
                With oMPListing
                    ...
                    'Added by Kavita Zinage 21/07/2015
                    chkAti.Checked = CBool(.GetValue("ATI__c"))
                    chkGraduate.Checked = CBool(.GetValue("Graduate__c"))
                    chkSchoolLeaver.Checked = CBool(.GetValue("SchoolLeaver__c"))
                End With
        ...
        Private Sub PopulateListingTypes()
            'Dim sSQL As String 'Commented by Kavita
            Dim dt As DataTable
            Dim i As Integer
            Dim oPrice As IProductPrice.PriceInfo

            Try
                'Commented by Kavita Inline query replaced with Stored Procedure
                'sSQL = "SELECT ID,Name,ProductID,'' NameWPrice FROM " & _
                '       Database & _
                '       "..vwMarketPlaceListingTypes "

                'Added by Kavita Zinage on 21/07/2015
                Dim sSQL As New StringBuilder()
                sSQL.AppendFormat("{0}..spGetMarketListingTypesbyUserID__c @WebUserID={1}", Me.Database, Me.User1.UserID)
                dt = DataAction.GetDataTable(sSQL.ToString(), Aptify.Framework.DataServices.IAptifyDataAction.DSLCacheSetting.BypassCache)

                For i = 0 To dt.Rows.Count - 1
                    oPrice = Me.GetProductPrice(CLng(dt.Rows(i).Item("ProductID")))
                    If IsDBNull(oPrice.Price) Then
                        oPrice.Price = CDec(0.0)
                    End If
        ...
        Public Property HTMLDescription() As String
            ...
            End Set
        End Property
        'Added new property for ATI,Graduate,School Leaver Checkbox
        Public Property ATI() As Boolean
            Get
                EnsureChildControls()
                Return chkAti.Checked
            End Get
            Set(ByVal Value As Boolean)
                chkAti.Checked = Value
            End Set
        End Property
        Public Property Graduate() As Boolean
            Get
                EnsureChildControls()
                Return chkGraduate.Checked
            End Get
            Set(ByVal Value As Boolean)              
                chkGraduate.Checked = Value
            End Set
        End Property
        Public Property SchoolLeaver() As Boolean
            Get
                EnsureChildControls()
                Return chkSchoolLeaver.Checked
            End Get
            Set(ByVal Value As Boolean)
                chkSchoolLeaver.Checked = Value
            End Set
        End Property 'Till here
