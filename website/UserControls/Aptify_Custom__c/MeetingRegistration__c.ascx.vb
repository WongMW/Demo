...
'Rajesh Kardile             04/07/2014                  Removed hard code message and add it to Culture Strings
'Sheela Jarali              31/05/2018                  Task #19380: Extension of Task #19167 - Shopping process performance
'Sheela Jarali              08/06/2018                  Bug #18439: Waiting List On Meeting To be Disabled URGENT 
'****************************************************************************************************
...
Namespace Aptify.Framework.Web.eBusiness.ProductCatalog
    Partial Class MeetingRegistrationControl__c
        ...
        'Added By Sheela as part of Task #19380
        Protected webname As String = String.Empty
        Protected AVAILSPACE As Integer = 0
        Protected LedgerID As Integer = -1
        Protected RequireInventory As Integer = 0
...
#Region "MeetingRegistration Specific Properties"
        ...
        'Modified By Sheela as part of Task #19380
        'Added below 3 Properies for webname,AVAILABLESPACE,LedgerID
        Public Property ProductWebName() As String
            Get
                Return webname
            End Get
            Set(ByVal value As String)
        ...
        Public Property MeetingAVAILSPACE() As Integer
            Get
                Return AVAILSPACE
            End Get
            Set(ByVal value As Integer)
        ...
        Public Property ProductLedgerID() As Integer
            Get
                Return LedgerID
            End Get
            Set(ByVal value As Integer)
        ...
        Public Property IsRequireInventory() As Integer
            Get
...
#End Region
        ...
        Protected Overrides Sub SetProperties()
            ...
            'Added by Sheela as part of Task #19380
            If String.IsNullOrEmpty(ProductWebName) Then
                ProductWebName = ""
                If Not String.IsNullOrEmpty(ProductWebName) Then
                    webname = ProductWebName
                End If
            End If

            If MeetingAVAILSPACE > 0 Then
                AVAILSPACE = MeetingAVAILSPACE
            Else
                AVAILSPACE = 0
            End If

            If ProductLedgerID > 0 Then
                LedgerID = ProductLedgerID
            Else
                LedgerID = -1
            End If
            '
            'Added by sheela as part Bug #18439
            If IsRequireInventory > 0 Then
            ...
            If Not IsPostBack Then
                ...
                If User1.UserID > 0 Then
                    If Request.QueryString("ID") IsNot Nothing AndAlso CInt(Request.QueryString("ID")) <> -1 AndAlso IsNumeric(Request.QueryString("ID")) Then
                        ...
                        'Added GetMeetingDetails() by Sheela as part of Task #19380
                        GetMeetingDetails()
                        GetMeetingInfo()
                        'Added by sheela as part Bug #18439
                        Session("CurrentAvailableSpace") = MeetingAVAILSPACE
                        Session("TotalAvailableSpace") = MeetingAVAILSPACE
                        Session("RequireInventory") = IsRequireInventory
                        If Convert.ToInt32(Session("RequireInventory")) = 1 Then
                            If Not Convert.ToInt32(Session("CurrentAvailableSpace")) > 0 Then
                                addMoreAttendees.Visible = False
                                lblAvailSpace.Visible = True
                                lblAvailSpace.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.MeetingRegistration.NoAvailableSpacesMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                            End If
                        End If
            ...
            If oOrderLine IsNot Nothing Then
                ...
                If iProductID > 0 Then
                    If "Meeting" = ShoppingCart1.GetProductType(iProductID) Then
                        ...
                        'Modified by Sheela to get ProductWebName property as part of Task #19380
                        lblMeeting.Text = ProductWebName ''GetProductWebName(iProductID)
                        ...
                        ''RashmiP,
                        'Modified By Sheela to get LedgerID as part of Task #19380
                        If ProductLedgerID > 0 Then
                            Dim lAvailableSpace As Integer = MeetingAVAILSPACE
                        ...
                        If oMeetingDetail IsNot Nothing Then
                            ...
                            If lAttendeeID > 0 Then
                                ...
                                sSQL = AptifyApplication.GetEntityBaseDatabase("Person") & "..spGetPersonDetail__c"
                                Dim param(0) As IDataParameter
                                param(0) = DataAction.GetDataParameter("@PersonID", SqlDbType.Int, lAttendeeID)
                                Dim dt = DataAction.GetDataTableParametrized(sSQL, CommandType.StoredProcedure, param)
                                ...
                                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                                    ...
                                    ' rdpBirthdate.SelectedDate = CDate((dt.Rows(0)("birthday"))) 'added for Redmine #20848 Rollback changes
                                Else
                                    txtFirstName.Text = User1.GetValue("FirstName")
                                    txtLastName.Text = User1.GetValue("LastName")
                                    txtEmail.Text = User1.GetValue("Email1")
                                    'Dim operson As AptifyGenericEntityBase = AptifyApplication.GetEntityObject("Persons", User1.PersonID) ' added by GM for Redmine #20848 Rollback changes
                                    'rdpBirthdate.SelectedDate = CDate(operson.GetValue("Birthday")) ' added by GM for Redmine #20848 Rollback changes
                                End If
                            End If
        ...
        Protected Overridable Sub MatchAndDeleteSession(ByVal OrderGE As OrdersEntity, ByVal ProductID As Long, ByVal AttendeeID As Long)
            Try
                ...
                sSQL = AptifyApplication.GetEntityBaseDatabase("Orders") & "..spGetSoldMeetingProduct__c"
                Dim param(0) As IDataParameter
        ...
        Protected Overridable Sub GetMeetingInfo()
            ...
            Try
                If ProductId <> -1 Then
                    'Modified By Sheela to get ProductWebName property as part of Task #19380
                    lblMeeting.Text = ProductWebName 'GetProductWebName(ProductId)
                    ...
                    'Modified By Sheela to get MeetingAVAILSPACE property as part of Task #19380
                    Dim lAvailableSpace As Integer = MeetingAVAILSPACE

                    'Modified By Sheela to get ProductLedgerID property as part of Task #19380
                    If ProductLedgerID > 0 Then
                        lblAvailableSpaceText.Text = "Available Space:"
                        lblAvailableSpace.Text = CStr(lAvailableSpace)
                        ...
                        Else
                            ...
                            lblMessage.Text = Convert.ToString(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.Meeting.NoSpaceForMeeting")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                            'Anil B for issue 15133 on 19-03-2013
        ...
        Private Function LoadMeetingDetail(ByVal MeetingDetailGE As AptifyGenericEntityBase) As Boolean
            ...
            If lAttendeeID > 0 Then
                ...
                txtLastName.Text = Trim(CStr(dt.Rows(0)("LastName")))
                txtEmail.Text = Trim(CStr(dt.Rows(0)("Email1")))
                'rdpBirthdate.SelectedDate = CDate((dt.Rows(0)("birthday"))) 'added for Redmine #20848
                'HP Issue#9090: determine if page is being called as a result of adding another attendee for the specified order line
            Else
                txtFirstName.Text = User1.GetValue("FirstName")
                txtLastName.Text = User1.GetValue("LastName")
                txtEmail.Text = User1.GetValue("Email1")
                'Dim operson As AptifyGenericEntityBase = AptifyApplication.GetEntityObject("Persons", User1.PersonID) ' added by GM for Redmine #20848
                'rdpBirthdate.SelectedDate = CDate(operson.GetValue("Birthday")) ' added by GM for Redmine #20848
            End If
        ...
        Private Function CheckForConflict(ByRef oOrder As OrdersEntity, ByVal oOrderLine As OrderLinesEntity, _
            ...
            Dim sConflictType As String = String.Empty
            Dim sSQL As String
            ...
            Dim param(0) As IDataParameter
            param(0) = DataAction.GetDataParameter("@productID", SqlDbType.Int, ProductID)
            sConflictType = CStr(DataAction.ExecuteScalarParametrized(sSQL, CommandType.StoredProcedure, param))
            Return False
        End Function

        Private Sub CleanUpSessions(ByVal lProductID As Long, ByVal iLine As Integer)
            'ShoppingCart1.RemoveItem(iLine)
            ShoppingCart1.SaveCart()
            ShoppingCart1.AddToCart(lProductID)
            ShoppingCart1.SaveCart()
        End Sub
        ...
        Private Function DoesConflictExist(ByVal lPersonID As Long, ByVal lProductID As Long, _
            ...
            With oOrder.SubTypes("OrderLInes")
                For i = iStart To iEnd
                    ...
                    'iProductID = CInt(.Item(i).GetValue("ProductID"))
                    'sSQL = AptifyApplication.GetEntityBaseDatabase("Products") & "..spGetMeetingDetails__c"
                    'Dim param(0) As IDataParameter
                    'param(0) = DataAction.GetDataParameter("@ProductId", SqlDbType.Int, iProductID)
                    ...
                    If dtMeeting IsNot Nothing AndAlso dtMeeting.Rows.Count > 0 Then
                        ...
                        End If
                        'Modified By Sheela as part of Task #19380 (Replaced inline query to SP)
                        'sSQL = "Select m.ID, m.StartDate, m.EndDate from " & Me.AptifyApplication.GetEntityBaseDatabase("Products") &
                        '        "..vwProducts p join " & Me.AptifyApplication.GetEntityBaseDatabase("Meetings") &
                        ...
                        'param1(0) = DataAction.GetDataParameter("@ProductId", SqlDbType.Int, lProductID)
                        'dt2 = DataAction.GetDataTableParametrized(sSQL, CommandType.StoredProcedure, param1)

                        If dtMeeting IsNot Nothing AndAlso dtMeeting.Rows.Count > 0 Then
                            dCurStart = CDate(dtMeeting.Rows(0).Item("StartDate"))
                            dCurEnd = CDate(dtMeeting.Rows(0).Item("EndDate"))
                            dOLStart = CDate(dtMeeting.Rows(0).Item("StartDate"))
                            dOLEnd = CDate(dtMeeting.Rows(0).Item("EndDate"))
                    ...
                    For i = iLine + 1 To .Count - 1
                        ...
                        If dtMeeting IsNot Nothing AndAlso dtMeeting.Rows.Count > 0 Then
                            ...
                            If dtMeeting IsNot Nothing AndAlso dtMeeting.Rows.Count > 0 Then
                                dCurStart = CDate(dtMeeting.Rows(0).Item("StartDate"))
                                dCurEnd = CDate(dtMeeting.Rows(0).Item("EndDate"))
                                dOLStart = CDate(dtMeeting(0).Item("StartDate"))
                                dOLEnd = CDate(dtMeeting.Rows(0).Item("EndDate"))
                                'Navin Prasad Issue 11488
        ...
        Private Sub SetInitialColumn()
            ...
            'dtAttendeeInfo.Columns.Add(New DataColumn("DateOfBirth", GetType(String))) ' added by GM for Redmine #20848
        ...
        Private Sub AddNewRowToGridFromAttendee(ByVal RowIndex As Integer)
            Try
                ...
                If Session(ATTRIBUTE_ATTENDEE_SESSION) IsNot Nothing Then
                    ...
                    'dtAttendeeInfo.Rows(iRowIndex)("DateOfBirth") = IIf(RowIndex = -1, rdpBirthdate.SelectedDate, rdpBirthdate.SelectedDate) 'added by GM for Redmine #20848
                ...
                Try
                    If Session(ATTRIBUTE_ATTENDEE_SESSION) IsNot Nothing Then
                        ...
                        If dt.Rows.Count > 0 Then
                            ...
                            'Added by sheela as part Bug #18439
                            If Convert.ToInt32(Session("RequireInventory")) = 1 Then
                                Session("CurrentAvailableSpace") = Convert.ToInt32(Session("TotalAvailableSpace")) - dt.Rows.Count
                                If Not Convert.ToInt32(Session("CurrentAvailableSpace")) > 0 Then
                                    addMoreAttendees.Visible = False
                                    lblAvailSpace.Visible = True
                                    lblAvailSpace.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.MeetingRegistration.NoAvailableSpacesMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                                Else
                                    addMoreAttendees.Visible = True
                                    lblAvailSpace.Visible = False
                                End If
                            End If
                        Else
        ...
        Protected Sub btnAddInfo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddInfo.Click
            'If rdpBirthdate.SelectedDate.ToString <> "" Then 'added for Redmine #20848
            ...
            If Session("CloseWindow") Is Nothing OrElse Convert.ToString(Session("CloseWindow")) <> "yes" Then
                'Added by Deepika on 29/11/2017 for log#17573
                'To add the attendee.
                If icount = 0 Then
                    icount = -1
                End If
                'End here 
                If icount = 1 Then
                    ''radDuplicateUser.VisibleOnPageLoad = True ''commented by GM for Redmine #19958
                    '' Exit Sub ''commented by GM for Redmine #19958
                    radDuplicateUser.VisibleOnPageLoad = False
                    Session("CloseWindow") = "yes"
            ...
            ''If ValidateGridRecords(txtEmail.Text.Trim.ToString(), -1) Then   '' If condition commented by GM for Redmine #19958
            AddNewRowToGridFromAttendee(-1)
            ''Added by sheela as part Bug #18439
            If Convert.ToInt32(Session("RequireInventory")) = 1 Then
                Session("CurrentAvailableSpace") = Convert.ToInt32(Session("CurrentAvailableSpace")) - 1
                If Not Convert.ToInt32(Session("CurrentAvailableSpace")) > 0 Then
                    addMoreAttendees.Visible = False
                    lblAvailSpace.Visible = True
                    lblAvailSpace.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.MeetingRegistration.NoAvailableSpacesMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
            ...
            ''Else
            If Session("CloseWindow") Is Nothing OrElse Convert.ToString(Session("CloseWindow")) <> "yes" Then
                ''   radValidateGrdRec.VisibleOnPageLoad = True '' Code commented by GM for Redmine #20029
            End If
            ''End If
            'Else 'else part added for Redmine #20848
            'lblMeetingRegistrationError.Visible = True 'added for Redmine #20848
            'lblMeetingRegistrationError.Text = "Date of birth required." 'added for Redmine #20848
            'lblMeetingRegistrationError.ForeColor = Drawing.Color.Red 'added for Redmine #20848
            'End If
        End Sub
            ...
            Try
                If ValidateAttendeeFromEditPopUp() Then
                    ...
                    If Not ValidateGridRecords(txtPopEmail.Text.Trim.ToString(), index) Then
                        '' Commented code by GM for Redmine #20029
        ...
        Protected Overridable Sub ClearText()
            Try
                ...
                'rdpBirthdate.SelectedDate = Nothing 'added for Redmine #20848
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
            End Try
        End Sub
        Protected Sub lnkMeetingPage_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkMeetingPage.Click
            If ViewState(ATTRIBUTE_ATTENDEE_MEETINGPRODUCTID) IsNot Nothing Then
                ...
                Me.Response.Redirect(sEventURl, True)
                Me.Dispose()
                Return
        ...
        End Sub
        ''' <summary>
        ''' added sDateOfBirth by GM for Redmine #20848
        ''' </summary>
        ...
        Protected Overridable Function FindCreatePerson(ByVal FirstName As String, ByVal LastName As String, ByVal Email As String) As Long
            Try
                ...
                Else
                    ...
                    'oPerson.SetValue("Birthday", sDateOfBirth)  ' added by GM for Redmine #20848
        ...
        Protected Sub btnAddRegistrant_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddRegistrant.Click
            ...
            If grdAddMember.Rows.Count > 0 Then
                ...
                oOrderLine = CType(oOrder.SubTypes("OrderLines").Find("ProductID", Request.QueryString("ID")), Aptify.Applications.OrderEntry.OrderLinesEntity)
                oOrderLine.SetValue("IsRequiredAgreement__c", Request.QueryString("req"))
                oOrderLine.SetValue("TermsAndConditionID__c", Request.QueryString("Term"))
        ...
        Protected Function AddRegistrants(ByVal IrowIndex As Integer, ByVal iCount As Integer, ByVal OrderGE As Aptify.Applications.OrderEntry.OrdersEntity) As Boolean
            Try
                ...
                'Dim sDateOfBirth As String = ""   'added by GM for Redmine #20848
                ...
                With grdAddMember
                    ...
                    'sDateOfBirth = CType(.Rows(IrowIndex).FindControl("lblDateOfBirth"), Label).Text 'added by GM for Redmine #20848
                    sAttendeeID = Convert.ToString(FindCreatePerson(CType(.Rows(IrowIndex).FindControl("lblAtendeeFirstName"), Label).Text, CType(.Rows(IrowIndex).FindControl("lblAtendeeLastName"), Label).Text, sAttendeeEmail)) 'added sDateOfBirth by GM for Redmine #20848 sDateOfBirth
        ...
        Protected Function LoadBadgeInfo(ByVal lAttendeeID As Long) As DataTable
            ...
            sSQL = "SELECT VP.FirstLast,VP.Title,VC.Name As CompanyName   FROM " & AptifyApplication.GetEntityBaseDatabase("Persons") & ".." & AptifyApplication.GetEntityBaseView("Persons") & " VP Left Outer join " & AptifyApplication.GetEntityBaseDatabase("Companies") & ".." & AptifyApplication.GetEntityBaseView("Companies") & " VC on VP.CompanyID =VC.ID Where VP.ID =" & lAttendeeID
            ...
            Try
                If Request.QueryString("OL") IsNot Nothing AndAlso CInt(Request.QueryString("OL")) <> -1 AndAlso IsNumeric(Request.QueryString("OL")) Then
                    ...
                    ElseIf icount = 0 Then
                        ...
                        Exit Sub
                    End If
                    Dim sAttendeeID As String = Convert.ToString(FindCreatePerson(txtFirstName.Text, txtLastName.Text, txtEmail.Text)) ' added by GM for Redmine #20848, CStr(rdpBirthdate.SelectedDate)
        ...
        Private Function GetConflictionOptionByProductID(ByVal MeetingProductID As Long) As String
            Dim sSQL As String
            'Modified By Sheela as part of Task #19380 (Replaced inline query to SP)P
            ...
            sSQL = AptifyApplication.GetEntityBaseDatabase("Meetings") & "..spCheckForMeetingConfliction__c"
            Dim param(0) As IDataParameter
            param(0) = DataAction.GetDataParameter("@productID", SqlDbType.Int, MeetingProductID)
            Dim sConfliction As String = CStr(DataAction.ExecuteScalarParametrized(sSQL, CommandType.StoredProcedure, param))
            Return sConfliction
        End Function
        'Anil B for Issue 14381
        ...
        Private Sub GetMeetingDates(ByVal vProductID As String, ByRef Startdate As DateTime, ByRef Enddate As DateTime)
            ...
            'sSQL = AptifyApplication.GetEntityBaseDatabase("Products") & "..spGetMeetingDetails__c"
            'Dim param(0) As IDataParameter
            'param(0) = DataAction.GetDataParameter("@ProductId", SqlDbType.Int, CInt(vProductID))
            'dtGetMeeting = DataAction.GetDataTableParametrized(sSQL, CommandType.StoredProcedure, param)
            ...
            For Each MeetingSession As KeyValuePair(Of Integer, Boolean) In dicMeetingSessions
                If MeetingSession.Value = True And strProduct.Trim() <> MeetingSession.Key.ToString() Then
                    ...
                    If CheckSessionConfliction(newStartdate, newEndDate, nextStartdate, nextEndDate) Then
                        ...
                        'sSQL = AptifyApplication.GetEntityBaseDatabase("Products") & "..spGetMeetingDetails__c"
                        'Dim param(0) As IDataParameter
                        'param(0) = DataAction.GetDataParameter("@ProductId", SqlDbType.Int, CInt(strProduct))
                        'dtGetMeeting = DataAction.GetDataTableParametrized(sSQL, CommandType.StoredProcedure, param)
        ...
        Private Function LoadUserMeetingStatus() As DataTable
            ...
            Dim iProductID As Integer, FirstName As String, LastName As String, Email As String
            ...
            sSQL = AptifyApplication.GetEntityBaseDatabase("OrderMeetingDetail") & "..spGetUserMeetingStatus__c"
            Dim param(3) As IDataParameter
            param(0) = DataAction.GetDataParameter("@FirstName", SqlDbType.VarChar, FirstName)
            param(1) = DataAction.GetDataParameter("@LastName", SqlDbType.VarChar, LastName)
            param(2) = DataAction.GetDataParameter("@Email", SqlDbType.VarChar, Email)
            param(3) = DataAction.GetDataParameter("@ProductID", SqlDbType.Int, iProductID)
            dtMeetingStatusDetails = DataAction.GetDataTableParametrized(sSQL, CommandType.StoredProcedure, param)
        ...
        Private Sub AddGoogleTagAddToCartClick()
            ...
            Dim ProductId As Long = CType(ViewState(ATTRIBUTE_ATTENDEE_MEETINGPRODUCTID), Long)

            Dim productName As String = GetProductWebName(ProductId)
            ...
            addedToShoppingCart.RenderWithWrapperFunction("addToCartEvent", AddressOf ReplaceQuantity)
        End Sub

        Private Function ReplaceQuantity(ByVal script As String) As String

            Return script.Replace("'quantity': 1", "'quantity': $('#grdAddMember tr').size() - 1")

        End Function

        'Added By Sheela as part of Task #19380 
        Private Sub GetMeetingDetails()
            Dim ProductId As Long = CType(Session("OL"), Long)
            Dim sSQL As String = AptifyApplication.GetEntityBaseDatabase("Products") & "..spGetMeetingDetails__c"
            Dim param(0) As IDataParameter
            param(0) = DataAction.GetDataParameter("@ProductId", SqlDbType.Int, ProductId)
            dtMeeting = DataAction.GetDataTableParametrized(sSQL, CommandType.StoredProcedure, param)
            If dtMeeting IsNot Nothing AndAlso dtMeeting.Rows.Count > 0 Then
                If String.IsNullOrEmpty(Trim(CStr(dtMeeting.Rows(0)("WebName")))) Then
                    ProductWebName = ""
                Else
                    ProductWebName = Trim(CStr(dtMeeting.Rows(0)("WebName")))
                End If
                MeetingAVAILSPACE = CInt(dtMeeting.Rows(0)("AVAILSPACE"))
                ProductLedgerID = CInt(dtMeeting.Rows(0)("IledgerID"))
                'Added by sheela as part Bug #18439
                IsRequireInventory = Convert.ToInt32(dtMeeting.Rows(0)("RequireInventory"))
            End If
        End Sub
