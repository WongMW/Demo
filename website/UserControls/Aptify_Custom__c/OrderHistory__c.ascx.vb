...
#End Region
        ...
        Private Sub LoadGrid()
            ...
            Try
                ...
                '    sSQL = sSQL & " AND OrderTypeID=" & cmbOrderType.SelectedItem.Value
                'End If
                ''Inner Join vwPaymentDetail pd on  pd.OrderID =o.ID
                ''sSQL = sSQL & " ORDER BY o.OrderDate DESC, o.ID DESC"
                'sSQL = sSQL & " AND RTRIM(ost.Name) = 'Shipped' AND OrderDate >= DATEADD(YEAR, -3, GETDATE()) " & " ORDER BY o.OrderDate DESC, o.ID DESC"

                Dim OrderType As Integer = 0
                If Convert.ToInt32(cmbOrderType.SelectedValue.Trim()) > 0 Then
                    OrderType = Convert.ToInt32(cmbOrderType.SelectedValue.Trim)
                End If


                sSQL = Database & "..spGetPastOrderHistory__c @PersonID=" & User1.PersonID & ",@OrderTypeID=" & Convert.ToString(OrderType)
        ...
        Protected Sub btnPrint_Click(sender As Object, e As System.EventArgs) Handles btnPrint.Click
            Try
                ...
                rptParam.Param1 = Convert.ToString(User1.PersonID) 'Aptify.Framework.Web.Common.WebCryptography.Decrypt(Request.QueryString("ID"))
                rptParam.SubParam1 = Convert.ToString(User1.PersonID)
                    ...
                    If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                        If dt.Rows.Count > 0 Then
                            If CInt(dt.Rows(0)(0)) = 1 Then
                                ...
                                Dim ReportID As Integer = Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("CrystalReportDetails__c", "Flexible Student Invoice Report")) 'Updated code added GM on 5th June 2018 for Redmine #18850
                                Dim rptParam As New AptifyCrystalReport__c
                                rptParam.ReportID = ReportID
                                rptParam.Param1 = InvoiceNo 'Updated code added GM on 5th June 2018 for Redmine #18850
                                Session(Me.AptifyApplication.GetEntityAttribute("CrystalReportDetails__c", "ParameterSession").Trim.ToLower()) = rptParam
