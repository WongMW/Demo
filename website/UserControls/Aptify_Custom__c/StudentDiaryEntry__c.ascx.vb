...
'Sheela Jarali	      23/07/2018		      Student able to make CA Diary entries before any mentor is assigned
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Option Strict On
Option Explicit On
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                ...
                lblAuditFieldMsg.text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.AuditingMsg")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                AddColumn()
                LoadLeaveTypes()
            ...
            If (lblStatus.Text = "Submitted to Mentor") Then 'Added by Harish Redmine Enhancement #20870 - To hide Submit to Mentor button when Student edit the Student diary entry
...
#Region "Functions"
    ...
    Private Sub AddColumn()
        Try
            ...
            'dtLeave.Columns.Add(New DataColumn("TOILDays", GetType(Double)))
    ...
    Public Function AddCADiaryRecord(ByVal Status As String) As Boolean
        Try
            ...
            oStudentDiaryGE.SetValue("StatutoryAuditDays", txtStatutoryDays.Text)
            oStudentDiaryGE.SetValue("CompanyAuditDays", txtCompanyDays.Text)
            oStudentDiaryGE.SetValue("OtherAuditDays", txtOtherDays.Text)
            'Added for redmine log #20233
            oStudentDiaryGE.SetValue("OtherAuditNIDays", txtOtherNIDays.Text)
            'End #20233
            If txtToilDays.Text.Trim = "" Then
            Else
                oStudentDiaryGE.SetValue("TOILDays", Convert.ToInt32(txtToilDays.Text))
            End If
            ...
            Try
                If oStudentDiaryGE.Save(False, _error) Then
                    ...
                    ' Code added by GM for Redmine #20073
                    If Status = "Submitted to Mentor" Then
                        Session("CallSubmittedToMentor") = "Yes"
                        ' Code added by GM for Redmine #20122
                        If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 Then
                            Session("CADairyID") = Convert.ToInt32(hdnCADiaryRecordID.Value)
                        End If
                        ''End Code for Redmine #20122
                    End If
                    ' End Redmine #20073
                    Return True
                Else
    ...
    Private Sub clearData()
        Try
            ...
            'Added for redmine log #18753
            txtStatutoryDays.Text = String.Empty
            txtCompanyDays.Text = String.Empty
            txtOtherDays.Text = String.Empty
            txtOtherNIDays.Text = String.Empty ' Redmine #20233
            ViewState("dtLeave") = Nothing
            ViewState("dtCompetency") = Nothing
    ...
    Private Sub DisableControl()
        Try
            ...
            txtOtherNIDays.ReadOnly = True ' Redmine #20233   
    ...
    Private Sub EnableControl()
        Try
            ...
            'Added for redmine log #18753
            txtStatutoryDays.ReadOnly = False
            txtCompanyDays.ReadOnly = False
            txtOtherDays.ReadOnly = False
            txtOtherNIDays.ReadOnly = False 'Redmine #20233
    ...
    Private Sub LoadDiaryDetails()
        Try
            ...
            If Not dtDiaryDetails Is Nothing AndAlso dtDiaryDetails.Rows.Count > 0 Then
                ...
                txtToilDays.Text = Convert.ToString(dtDiaryDetails.Rows(0).Item("TOILDays")) ' 17937
                txtOtherNIDays.Text = Convert.ToString(dtDiaryDetails.Rows(0).Item("OtherAuditNIDays")) ' Redmine #20233
            End If
            LoadLeavesGrid()
            LoadCompetencyGrid()
            'Added by Kavita Zinage 15/03/2016
...
Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
    ...
    Private Sub ValidateData()
        Try
            ...
            ' above code aommented by GM AND added below line for redmine #18771
            Dim sIntakedateSQL As String = Me.Database & "..spCheckEducationContractate__c @PersonID=" & Convert.ToInt32(AptifyEbusinessUser1.PersonID) & ""
            ...
            If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                Else
                    If Not dtDiary Is Nothing AndAlso dtDiary.Rows.Count > 0 Then
                        If Convert.ToString(dtDiary.Rows(0).Item("Status")).ToLower() = "locked" Then 'Or Convert.ToString(dtDiary.Rows(0).Item("Status")).ToLower() = "submitted to mentor" Then 'Modified by Harish Redmine Enhancement #20870 - disable controls on page only if the status is Locked
                            DisableControl()
                        End If
                    End If
                End If
            ...
            If Not dtDiary Is Nothing AndAlso dtDiary.Rows.Count > 0 Then
                If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 And Convert.ToString(dtDiary.Rows(0).Item("Type")).ToLower() = "prior work experience" Then
                    ...
                    ''Added By Pradip 2017-04-24 for red mine issue https://redmine.softwaredesign.ie/issues/17270
                    Dim dtattached As DataTable = Nothing
                    dtattached = CType(CType(Me.RecordAttachments__c.FindControl("grdAttachments"), RadGrid).DataSource, DataTable)

                    If dtattached.Rows.Count > 0 Then
                        PriorExp.Visible = True
                    Else
                        PriorExp.Visible = False
                    End If
...
#Region "Button Click"
        ...
        Try
            If (ddlLeaveType.SelectedItem.Text <> "" Or ddlLeaveType.SelectedItem.Text <> "Select") AndAlso txtDays.Text <> "" Then
                ...
                If checkCount = 0 Then
                    ...
                    dr("Days") = Math.Round(Convert.ToDouble((txtDays.Text)), 0, MidpointRounding.AwayFromZero) ' https://redmine.softwaredesign.ie/issues/17457
                    'dr("TOILDays") = Convert.ToDouble(txtToilDays.Text)
        ...
        Try
           ...
           Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.ErrorMsg1")), _
                ...
                If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                    ...
                    'Code added https://redmine.softwaredesign.ie/issues/17937
                    Dim drComptencyCategory() As DataRow = dt.Select("ComptencyCategory = 'Auditing'")
                    If drComptencyCategory.Length > 0 Then
                        'Added for redmine log #18753
                        txtStatutoryDays.Enabled = True
                        txtCompanyDays.Enabled = True
                        txtOtherDays.Enabled = True
                        txtOtherNIDays.Enabled = True ' Redmine #20233
                    Else
                        Dim drComptencyCategoryOther() As DataRow = dt.Select("ComptencyCategory = 'Auditing (Other)'")
                        If drComptencyCategoryOther.Length > 0 Then
                            'Added for redmine log #18753
                            txtStatutoryDays.Enabled = True
                            txtCompanyDays.Enabled = True
                            txtOtherDays.Enabled = True
        ...
        Try
            ...
            If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                Else
                    ...
                    Else
                        'Added for redmine log #18753
                        txtStatutoryDays.Enabled = False
                        txtStatutoryDays.Text = ""
                        txtCompanyDays.Enabled = False
                        txtOtherDays.Enabled = False
                        txtCompanyDays.Text = ""
                        txtOtherDays.Text = ""
                        txtOtherNIDays.Enabled = False 'Redmine #20233
                        txtOtherNIDays.Text = "" 'Redmine #20233
                    End If
                End If
            Else
                'Added for redmine log #18753
    ...
    ''' </summary>
    '''Protected Sub btnSavenExit_Click(sender As Object, e As System.EventArgs) Handles btnSavenExit.Click -- Commented by Harish Redmine Bug #20931 to call server side function/Method
    Protected Sub btnSavenExit_Click(sender As Object, e As System.EventArgs) '' Modified by Harish Redmine Bug #20931 to call server side/Client side function/Method at same time on save and exit button click
        If DaysValidation() Then ' if Condition added by GM for Redmine #20482
            ExperienceValidate()
            If BeforeSaveValidate() And Convert.ToBoolean(ViewState("bFail")) = True AndAlso Convert.ToBoolean(ViewState("bStartDateFail")) = True Then
                If Convert.ToInt32(HFMentorID.Value) > 0 Then
                    If ValidateCompanyandOtherAuditDays() = False Then ' Condtion for https://redmine.softwaredesign.ie/issues/17937 
                        ''Redmine Enhancement #20871 - Commented code for the log Enhancement #20871 - Modified by Harish Date. 14/11/2019
                        ''If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 Then
                        ...
                        ''        If Convert.ToInt32(hdnCADiaryRecordID.Value) < 0 Then
                        ''            lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(
                        ''                Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "Aptify.Ebusiness.StudentDiaryEntryPage.SuccessMsg__c")),
                        ''                Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                        ''        Else
                        ''            lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(
                        ''                Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "Aptify.Ebusiness.StudentDiaryEntryPage.SuccMsg__c")),
                        ''                Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                        ''        End If

                        ''    Else
                        ''        lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(
                        ''              Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.validmsg3")),
                        ''                            Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                        ''    End If
                        ...
                        ''Redmine Enhancement #20871 -Start of the Code - To get promt Message to student after Save * Exit button click for Competency 'Auditing' and 'Auditing (Other)' - Added by Harish Date. 14/11/2019
                        Dim drComptencyCategory() As DataRow = CType(ViewState("dtCompetency"), DataTable).Select("ComptencyCategory = 'Auditing'")
                        Dim drComptencyCategoryOther() As DataRow = CType(ViewState("dtCompetency"), DataTable).Select("ComptencyCategory = 'Auditing (Other)'")
                        If drComptencyCategory.Length > 0 OrElse drComptencyCategoryOther.Length > 0 Then
                            radWindowSave.VisibleOnPageLoad = True
                            lblSave.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "Aptify.Ebusiness.StudentDiaryEntryPage.AuditingORAuditingOtherElement__c")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
                        Else
                            If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 Then
                                If AddCADiaryRecord("With Student") Then
                                    Response.Redirect(ExistingDiaryPage)
                            ...
                            Else
                                If AddCADiaryRecord("With Student") Then
                                    ...
                                    lblMessage.ForeColor = Drawing.Color.Blue
                                    If Convert.ToInt32(hdnCADiaryRecordID.Value) < 0 Then
                                        lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "Aptify.Ebusiness.StudentDiaryEntryPage.SuccessMsg__c")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                                        '' Start of Code - Added by Harish Redmine Bug #20931 to Enable button
                                        btnSavenExit.Attributes.Add("disabled", "False")
                                        btnSavenExit.Attributes.Remove("disabled")
                                        btnSavenExit.Attributes.Add("class", "submitBtn")
                                        '' End of Code - Added by Harish Redmine Bug #20931 to Enable button
                                    Else
                                        lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(
                                            Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "Aptify.Ebusiness.StudentDiaryEntryPage.SuccMsg__c")),
                                            Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                        ...
                        End If
                        ''Redmine Enhancement #20871 -End of the Code - To get promt Message to student after Save * Exit button click for Competency 'Auditing' and 'Auditing (Other)' - Added by Harish Date. 14/11/2019
                    Else
                        lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal( _
                                                                   Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.TotalDaysCalcMsg")), _
                                                                   Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                        '' Start of Code - Added by Harish Redmine Bug #20931 to Enable button
                        btnSavenExit.Attributes.Add("disabled", "False")
                        btnSavenExit.Attributes.Remove("disabled")
            ...
            Else
                ...
                '' End of Code - Added by Harish Redmine Bug #20931 to Enable button
            End If
        Else 'else part added by GM for Redmine #20482
            lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(
                            Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.MaxNumberExperienceDaysValidation")),
                                Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
            '' Start of Code - Added by Harish Redmine Bug #20931 to Enable button
            btnSavenExit.Attributes.Add("disabled", "False")
            btnSavenExit.Attributes.Remove("disabled")
        ...
        End If 'end Redmine #20482
    End Sub
    ''' <summary>
    ''' Validates the companyand other audit days. https://redmine.softwaredesign.ie/issues/17937
    ''' </summary>
    ''' <returns></returns>
    Protected Function ValidateCompanyandOtherAuditDays() As Boolean
        Try
            'Added for redmine log #18753/20233
            If txtCompanyDays.Text <> "" OrElse txtOtherDays.Text <> "" OrElse txtStatutoryDays.Text <> "" OrElse txtOtherNIDays.Text <> "" Then
                If txtOtherDays.Text = "" Then
                    txtOtherDays.Text = "0"
                End If
                If txtCompanyDays.Text = "" Then
                    txtCompanyDays.Text = "0"
                End If
                'Added for redmine log #18753
                If txtStatutoryDays.Text = "" Then
                    txtStatutoryDays.Text = "0"
                End If
                'Added for redmine log #20233
                If txtOtherNIDays.Text = "" Then
                    txtOtherNIDays.Text = "0"
                End If
                If txtToilDays.Text <> "" Then
                    'Added StatutoryDays for redmine log #18753/#20233
                    'redmine log #18753: Made changes in formula below to compare sum of audit days with number of days of relevant experience pertaining to this diary entry instead of Max. number of experience days
                    If Convert.ToInt32(txtCompanyDays.Text) + Convert.ToInt32(txtOtherDays.Text) + Convert.ToInt32(txtStatutoryDays.Text) + Convert.ToInt32(txtOtherNIDays.Text) > Convert.ToInt32(txtExperience.Text) + Convert.ToInt32(txtToilDays.Text) Then
                        Return True
                    End If
                Else
                    'Added StatutoryDays for redmine log #18753 /#20233
                    'redmine log #18753: Made changes in formula below to compare sum of audit days with number of days of relevant experience pertaining to this diary entry instead of Max. number of experience days
                    If Convert.ToInt32(txtCompanyDays.Text) + Convert.ToInt32(txtOtherDays.Text) + Convert.ToInt32(txtStatutoryDays.Text) + Convert.ToInt32(txtOtherNIDays.Text) > Convert.ToInt32(txtExperience.Text) Then
                        Return True
                    End If
                End If
            End If
    ...
    Protected Sub btnSubmitToMentor_Click(sender As Object, e As System.EventArgs) Handles btnSubmitToMentor.Click
        Try
            'Added below line as part of #20101
            ExperienceValidate()
            If Convert.ToBoolean(ViewState("bFail")) = False Then
                radWindowSubmiToMentor.VisibleOnPageLoad = False
                lblexperr.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.DayValidation")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                Return
            End If
            'End
            If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 Then
            ...
            End If
            'Added as part of Bug #19658:Student able to make CA Diary entries before any mentor is assigned
            If Convert.ToInt32(HFMentorID.Value) > 0 Then
                Session("CallSubmittedToMentor") = Nothing ' line added by GMfor Redmine #20417 for Subsequent Submitted To Mentor
                'below code added for Redmine #20871
                Dim drComptencyCategory() As DataRow = CType(ViewState("dtCompetency"), DataTable).Select("ComptencyCategory = 'Auditing'")
                Dim drComptencyCategoryOther() As DataRow = CType(ViewState("dtCompetency"), DataTable).Select("ComptencyCategory = 'Auditing (Other)'")
                If drComptencyCategory.Length > 0 OrElse drComptencyCategoryOther.Length > 0 Then
                    lblMentor.Text = Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "Aptify.Ebusiness.StudentDiaryEntryPage.AuditingORAuditingOtherElement__c")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials)
            ...
            Else
                lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.validmsg2")), Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
            End If
        Catch ex As Exception
            Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
        End Try
    ...
    Protected Sub btnReviewYes_Click(sender As Object, e As System.EventArgs) Handles btnReviewYes.Click
        Try
            If DaysValidation() Then ' if Condition added by GM for Redmine #20482
                ...
                'Added by GM if condition for Redmine #20073/ ' updated If condition for Redmine #20122
                If Session("CallSubmittedToMentor") Is Nothing OrElse (Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 AndAlso Convert.ToInt32(hdnCADiaryRecordID.Value) <> Convert.ToInt32(Session("CADairyID"))) Then
                    If ValidateCompanyandOtherAuditDays() = False Then ' Condtion for https://redmine.softwaredesign.ie/issues/17937
                        If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 Then
                ...
                If Convert.ToString(Session("CallSubmittedToMentor")) = "Yes" Then

                    If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 Then
                        '' If condition  added by GM for Redmine #20122
...
#Region "Change Events"
    ...
    Protected Sub txtStartDate_SelectedDateChanged(sender As Object, e As Telerik.Web.UI.Calendar.SelectedDateChangedEventArgs) Handles txtStartDate.SelectedDateChanged
        Try
            ...
            If iCount > 0 Or dDate < dIntakeDate Then
                'ViewState("bFail") = False
                ViewState("bStartDateFail") = False
                lblMessage.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal( _
                            Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.validmsg1")), _
                            Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
    ...
    Protected Sub txtExperience_TextChanged(sender As Object, e As System.EventArgs) Handles txtExperience.TextChanged
        ...
        '        lblexperr.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(
        '                    Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.DayValidation")),
        '                    Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
        '    Else
...
#Region "Grid Events"
    ...
    Protected Sub gvCompetency_NeedDataSource(sender As Object, e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles gvCompetency.NeedDataSource
        ...
        End Try
    End Sub
    'Added as part of #20101
    Public Sub ExperienceValidate()
        Try
            If Not txtStartDate.SelectedDate Is Nothing And Not txtEndDate.SelectedDate Is Nothing Then
                ...
                Dim startDate As Date = Convert.ToDateTime(txtStartDate.SelectedDate)
                Dim endDate As Date = Convert.ToDateTime(txtEndDate.SelectedDate)

                If startDate <= endDate Then '' Modified by Harish - Redmine Bug #21248 not allowing to save the record having days 1 and same startdate and enddate -- before If startDate < endDate Then
                    oParams(0) = DataAction.GetDataParameter("@StartDate", SqlDbType.DateTime, Convert.ToDateTime(txtStartDate.SelectedDate))
                    oParams(1) = DataAction.GetDataParameter("@EndDate", SqlDbType.DateTime, Convert.ToDateTime(txtEndDate.SelectedDate))
                    sSQL = Me.Database & "..spGetDiaryExperienceDaysbyDates__c"
                    iTotalDiffDays = Convert.ToInt32(DataAction.ExecuteScalarParametrized(sSQL, CommandType.StoredProcedure, oParams))
                ...
                Else
                    ...
                    radWindowSubmiToMentor.VisibleOnPageLoad = False
                    lblexperr.Text = CStr(Aptify.Framework.BusinessLogic.CultureUtility.GetCultureLocal(
                            Convert.ToInt32(AptifyApplication.GetEntityRecordIDFromRecordName("Culture Strings", "AptifyEbusiness.StudentDiaryEntryPage.datevalidmsg_c")),
                            Convert.ToInt32(DataAction.UserCredentials.CultureID), DataAction.UserCredentials))
                End If
            End If
        Catch ex As Exception
...
#End Region
    ''' <summary>
    ''' Validation added for Redmine #20482
    ''' </summary>
    ''' <returns></returns>
    Private Function DaysValidation() As Boolean
        Try
            ' Dim days As Long = DateDiff("d", txtStartDate.SelectedDate, txtEndDate.SelectedDate)
            If Convert.ToDouble(txtExperience.Text.ToString()) <= Convert.ToDouble(TxtTotalDays.Text) Then
                Return True
            Else
                Return False
            End If
    ...
    Protected Sub btnSaveYes_Click(sender As Object, e As EventArgs) Handles btnSaveYes.Click
        If Convert.ToInt32(hdnCADiaryRecordID.Value) > 0 Then
            If AddCADiaryRecord("With Student") Then
                Response.Redirect(ExistingDiaryPage)
            End If
        Else
            If AddCADiaryRecord("With Student") Then '' Create regular Diary entry for student
                clearData()
                ValidateData()
                lblMessage.ForeColor = Drawing.Color.Blue
                If Convert.ToInt32(hdnCADiaryRecordID.Value) < 0 Then
        ...
        End If
        radWindowSave.VisibleOnPageLoad = False
    End Sub
    ''Redmine Enhancement #20871 - End of the Code - Added by Harish Date. 14/11/2019

    ''Redmine Enhancement #20871 - Start of the Code - Added by Harish Date. 14/11/2019
    Protected Sub btnSaveNo_Click(sender As Object, e As EventArgs) Handles btnSaveNo.Click
        Try
            radWindowSave.VisibleOnPageLoad = False ''Close the Promt message Window
        Catch ex As Exception
            Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
        End Try
    End Sub
