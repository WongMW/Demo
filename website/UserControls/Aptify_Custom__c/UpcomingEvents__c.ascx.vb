''' <summary>
''' Created By Pradip 2016-06-17 For Category wise Events.
''' </summary>
Option Explicit On
Option Strict On
Imports System.Data
...
Namespace Aptify.Framework.Web.eBusiness
    Partial Class UpcomingEvents__c
        ...
        Public Property EventCategory() As String
            Get
                If ViewState.Item("EventCategory") IsNot Nothing Then
                    Return ViewState.Item("EventCategory").ToString()
                Else
                    Return ""
                End If
            End Get
            Set(ByVal value As String)
                ViewState.Item("EventCategory") = value
            End Set
        End Property
        ...
        Private Sub LoadSchedule()
            ...
            Try
                ...
                sSQL = " SELECT top 5 p.ID ProductID,p.WebName,p.WebImage, p.Description,p.WebDescription, " & _
                       ...
                       sDB & "..vwMeetings m ON p.ID=m.ProductID  INNER JOIN " & _
                        sDB & "..vwProductMultipleCategories__c pm ON PM.ProductID = P.ID " & _
                       " INNER JOIN " & sDB & "..vwProductCategories pc " & _
                       " ON pc.id = pm.CategoryID " & _
                       " WHERE ISNULL(p.ParentID,-1)<=0 AND pc.WebEnabled=1 AND p.WebEnabled=1 " & _
                       "AND PC.WEBNAME IN(" & EventCategory & ")"
                ...
                If Not dt Is Nothing And dt.Rows.Count > 0 Then
                    If dt.Rows.Count = 5 And Me.ID = "UpcomingEventNew__c" Then
            ...
            Try
                If (e.Item.ItemType = ListItemType.Item) OrElse (e.Item.ItemType = ListItemType.AlternatingItem) Then
                    ...
                    If e.Item.FindControl("lblDate") IsNot Nothing AndAlso DataBinder.Eval(e.Item.DataItem, "StartDate") IsNot Nothing AndAlso Not IsDBNull(DataBinder.Eval(e.Item.DataItem, "StartDate")) AndAlso DataBinder.Eval(e.Item.DataItem, "EndDate") IsNot Nothing AndAlso Not IsDBNull(DataBinder.Eval(e.Item.DataItem, "EndDate")) Then
                        ...
                        Dim strStartDate As String = DataBinder.Eval(e.Item.DataItem, "StartDate", "{0:MMM dd}")
                        Dim strEndate As String = DataBinder.Eval(e.Item.DataItem, "EndDate", "{0:MMM dd}")
                        If strStartDate = strEndate Then
                            lbl.Text = DataBinder.Eval(e.Item.DataItem, "StartDate", "{0:MMM dd}")
                        Else
                            lbl.Text = DataBinder.Eval(e.Item.DataItem, "StartDate", "{0:MMM dd}") + " - " + DataBinder.Eval(e.Item.DataItem, "EndDate", "{0:MMM dd}")
                        End If
                    ...
                    If DataBinder.Eval(e.Item.DataItem, "Venue") IsNot Nothing AndAlso Not IsDBNull(DataBinder.Eval(e.Item.DataItem, "Venue")) AndAlso DataBinder.Eval(e.Item.DataItem, "Venue") IsNot String.Empty Then
                        If lbl IsNot Nothing Then
                            lbl.Text = CStr(DataBinder.Eval(e.Item.DataItem, "Venue"))
                        End If
                    End If
                ...
                End If
                If (e.Item.ItemType = ListItemType.Footer) And Not e.Item.FindControl("linkViewAll") Is Nothing Then
