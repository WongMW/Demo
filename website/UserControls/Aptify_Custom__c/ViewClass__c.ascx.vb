...
Namespace Aptify.Framework.Web.eBusiness.Education
    Partial Class ViewClassControl__c
...
#End Region
        ...
        Protected Overrides Sub SetProperties()
            ...
            If String.IsNullOrEmpty(LeftNavDocumentsImage) Then
                ...
                imgPreDocumentSmall.Src = LeftNavDocumentsImage
            End If
        ...
        Protected Overridable Function ValidateClassRegistration() As Boolean
            ...
            Try
                'sql = "SELECT TOP 1 ID, Status, DateAvailable, ISNULL(DateExpires,'') DateExpires FROM " & _
                ...
                sql = "..spGetClassRegData__c @ClassID=" + Request.QueryString("ClassID") & ",@StudentID=" & Me.InstructorValidator1.User.PersonID
                dt = Me.DataAction.GetDataTable(sql)
        ...
        Private Sub SetupView(ByVal InstructorID As Long, ByVal WebName As String, ByVal InstructorComments As String, ByVal WebDetails As String, ByVal ClassID As Long, ByVal ProductID As Long, Optional ByVal ProductType As String = "")
            ...
            lnkPreDownload.Font.Bold = False
            ...
            Select Case m_sView
                ...
                Case "Downloads" 'Rajesh K-04/19/2014
                    lnkPreDownload.Font.Bold = True
                    Me.pnlDownloads.Visible = True
                    Me.displayRecordAttachment(CLng(Request.QueryString("ClassID")))
                    lblTitle.Text = "Downloads for " & WebName
                    imgTitle.ImageUrl = DisplayDocumentsImage
            End Select
            ...
            lnkPreDownload.NavigateUrl = sBase & "Downloads" 'Rajesh K-04/19/2014
        End Sub
        ...
        Private Sub LoadSyllabus()
            Dim sql As String, dt As Data.DataTable, sDB As String
            Try
                ...
                sql = "..spGetClassSyllabus__c @ClassID=" & Me.Request("ClassID") & ", @ClassRegID=" & Me.ClassRegistrationID.ToString
                ...
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    ...
                    For i As Integer = 0 To dt.Rows.Count - 1
                        ...
                        If dt.Rows(i).Item("CourseStatus").ToString.ToLower.Trim <> "completed" _
                                ...
                                AndAlso dt.Rows(i).Item("CourseStatus").ToString.ToLower.Trim <> "failed" Then
                    ...
                    'For Each row As GridViewRow In grdSyllabus.Rows
                    '    Dim lnk As HyperLink = CType(row.FindControl("lnkWebName"), HyperLink)
                    '    lnk.NavigateUrl = String.Format("", dt.Rows((grdSyllabus.PageIndex * grdSyllabus.PageSize) + rowcounter)("WebURL").ToString)
                    '    'Dim lbl As Label = CType(row.FindControl("lblDuration"), Label)
                    '    'lbl.Text = String.Format("{0:F0} min", dt.Rows(row.RowIndex)("Duration").ToString)
                    'Next
                End If
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
        ...
        Private Sub LoadStudents()
            ...
            Try
                ...
                '    '    Dim lnk As HyperLink = CType(row.FindControl("lnkLastName"), HyperLink)
                '    '    lnk.NavigateUrl = String.Format("mailto:{0:s}", dt.Rows((grdStudents.PageIndex * grdStudents.PageSize) + rowcounter)("Email1").ToString)

                '    '    lnk = CType(row.FindControl("lnkFirstName"), HyperLink)
                '    '    lnk.NavigateUrl = String.Format("mailto:{0:s}", dt.Rows((grdStudents.PageIndex * grdStudents.PageSize) + rowcounter)("Email1").ToString)
                '    '    Dim lbl As Label
                '    '    lbl = CType(row.FindControl("lblScore"), Label)
                '    '    lbl.Text = String.Format("{0:d}", dt.Rows((grdStudents.PageIndex * grdStudents.PageSize) + rowcounter)("Score").ToString)
                ...
                '    grdStudents.DataSource = dt
                '    '' grdStudents.DataBind()
                '    'grdStudents.Rows(0).Visible = False
                'End If
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
            End Try
        ...
        Protected Sub grdSyllabus_ItemCommand(sender As Object, e As Telerik.Web.UI.GridCommandEventArgs) Handles grdSyllabus.ItemCommand
            If e.CommandName = "Upload" Then
                radUploadDocuments.VisibleOnPageLoad = True
                Dim data() As String
                data = e.CommandArgument.Split(";")
                Dim recordID As Integer = CInt(data(0))
                Dim entityID As Integer = CInt(data(1))
                ucUpload.EntityID = entityID
                ucUpload.RecordID = recordID
            End If
        ...
        Protected Sub grdStudents_PageIndexChanging(ByVal sender As Object, ByVal e As GridPageChangedEventArgs) Handles grdStudents.PageIndexChanged
            ...
            LoadStudents()
        End Sub
        Protected Sub grdStudents_PageIndexChanging(ByVal sender As Object, ByVal e As GridPageSizeChangedEventArgs) Handles grdStudents.PageSizeChanged
            ''grdStudents.PageIndex = e.NewPageIndex
            LoadStudents()
        End Sub
...
#Region "Custom Methods"
        ...
        ''' <remarks></remarks>
        Protected Sub displayRecordAttachment(ByVal classID As Long)
            Try
                trRecordAttachment.Visible = True
                Me.RecordAttachments__c.Visible = True
                Dim lEntityId As Long
                lEntityId = CLng(Me.AptifyApplication.GetEntityID("Classes"))
                LoadDataFromGE(Me.AptifyApplication.GetEntityObject("Classes", classID))
                ...
                Me.RecordAttachments__c.LoadAttachments(lEntityId, classID)
...
#End Region
        ...
        Protected Sub btnCancel_Click(sender As Object, e As System.EventArgs) Handles btnCancel.Click
            radUploadDocuments.VisibleOnPageLoad = False
        End Sub
