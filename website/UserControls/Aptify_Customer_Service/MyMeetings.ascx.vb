...
#End Region
        ...
        Protected Overridable Function GetSql() As String
            Try
                ...
                'Updated  query 
                sSQL = "Select O.MeetingName,P.WebName,O.AttendeeID,o.OrderID,M.ID as MeetingID, O.AttendeeID_FirstLast,O.RegistrationType,S.Name Status,M.place Location,O.ProductID,M.StartDate,M.EndDate " &
                                           "from " & sDB & "..vwOrderMeetDetail O inner join " & sDB & "..vwMeetings M on O.ProductID= M.ProductID " &
                                           "inner join " & sDB & "..vwAttendeeStatus S on O.StatusID=S.ID " &
                                           "inner join " & sDB & "..vwproducts P on P.ID=O.ProductID " &
            ...
            Try
                ...
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    For Each dr In dt.Rows
                        'Modified as part of #20443
                        'Changed delemeter : to ^ #20650
                        Dim notificationDesc As String = dr("WebName") & "^ Date of Event is " & dr("StartDate") & If(dr("StartDate") <> dr("EndDate"), "  to  " & dr("EndDate"), "") & "   Your Status- " & dr("AttendeeStatus_Name")
                        ' below line commented and added new line for Desc for Redmine #20655  
                        'Dim Desc As String = dr("WebName") & "^ Date of event is " & dr("StartDate") & If(dr("StartDate") <> dr("EndDate"), "  to  " & dr("EndDate"), "") & "   <span class='event_meeting_regstatus'><i class='fas fa-user-circle'></i> " & dr("AttendeeStatus_Name") & " - You are registered to attend this event</span>"
                        Dim Desc As String = dr("WebName") & "^ Date of event is " & dr("StartDate") & If(dr("StartDate") <> dr("EndDate"), "  to  " & dr("EndDate"), "") & "   <span class='event_meeting_regstatus'><i class='fas fa-user-circle'></i> " & dr("AttendeeStatus_Name") & " - You are registered to attend this event</span>	<span class='addtocalendar'><var class='atc_event'><var class='atc_date_start'>" & dr("StartDate") & " </var> <var class='atc_date_end'>" & dr("EndDate") & " </var> <var class='atc_timezone'>Europe/London</var> <var class='atc_title'>" & dr("WebName") & " </var> <var class='atc_description'>" & dr("VerboseDescription") & " </var><var class='atc_location'>" & dr("Location") & " </var><var class='atc_organizer'>" & dr("Coordinator") & " </var> <var class='atc_organizer_email'>no-reply@charteredaccountants.ie</var></span>"
                        dr("Description") = notificationDesc
                        drNotifications = dtUpcomingMeetingNotification.NewRow()
                        drNotifications("Description") = Desc
                        'End of #20443
                ...
                If dtMeetingsForYouNotification IsNot Nothing AndAlso dtMeetingsForYouNotification.Rows.Count > 0 Then
                    ...
                    'Susan Wong, #19497
                    lblNotificationForYou.Text = "You have <span>" & dtMeetingsForYouNotification.Rows.Count & "</span> upcoming events"
                    iTotalNotificationCount = iTotalNotificationCount + dtMeetingsForYouNotification.Rows.Count
                Else
                    'Susan Wong, #1949
                    lblNotificationForYou.Text = "You have no upcoming events"
                    iTotalNotificationCount = iTotalNotificationCount + 0
                End If
