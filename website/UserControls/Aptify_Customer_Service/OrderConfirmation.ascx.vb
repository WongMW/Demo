...
#End Region
        ...
        Private Sub LoadOrderHeader()
            ...
            Try
                If Not IsNumeric(Request.QueryString("ID")) Then
                    Throw New ArgumentException("Parameter must be numeric.", "ID")
                End If
                sSQL = "SELECT o.*,ost.Name OrderStatus FROM " &
                       Database &
                       "..vwOrders o INNER JOIN " &
                       Database &
                       "..vwOrderStatusTypes ost ON " &
            ...
            End Try
        End Sub
        Private Sub SetAddress(ByVal oBlock As NameAddressBlock,
        ...
        Private Sub LoadOrderDetails()
            ...
            Try
                sSQL = "SELECT *, case when (P.WebName is null or P.WebName = '') 	then P.Name 	Else P.WebName End as WebName FROM " &
                       AptifyApplication.GetEntityBaseDatabase("OrderLines") & ".." & sOrderDetailView & " OD inner join " & AptifyApplication.GetEntityBaseDatabase("Products") & ".." & sProductView & " P on OD.ProductID=P.id " &
                       "WHERE OrderID=" &
        ...
        Private Function SetURLPerProductType(ByVal productId As Integer, ByVal extId As Integer) As String
            ...
            If Not IsNothing(p.GetValue("ProductType")) AndAlso p.GetValue("ProductType").ToString.ToUpper = "CLASS" Then
                sql = "SELECT TOP 1 ClassID FROM " & AptifyApplication.GetEntityBaseDatabase("ClassRegistrations") &
        ...
        Protected Sub grdMain_RowDataBound(ByVal sender As Object, ByVal e As GridItemEventArgs) Handles grdMain.ItemDataBound
            If (TypeOf (e.Item) Is GridDataItem) Then
                ...
                DirectCast(e.Item.FindControl("link"), HyperLink).NavigateUrl =
        ...
        Private Function DoesUserHaveAccessOnOrder(ByVal UserId As Long, ByVal OrderID As Long) As Boolean
            Try
                ...
                sSQL = "SELECT COUNT(O.ID)  from " &
                       Database & ".." & AptifyApplication.GetEntityBaseView("Persons") & " P " &
                            ", " & Database & ".." & AptifyApplication.GetEntityBaseView("Orders") & " O " &
                          " Where ((P.IsGroupAdmin = 1 AND O.BillToCompanyID = P.CompanyID AND P.ID = " & UserId & ") " &
                              " OR ( O.BillToID = " & UserId & "))" &
                                 " AND O.ID =" & OrderID
        ...
        Private Sub AddGTMPurchase()
            ...
            purchaseDto = New PurchaseDto() With {
                .TransactionId = headerRow("ReferenceTransactionNumber").ToString,
                .Affiliattion = "",
            ...
            For Each detailRow As Data.DataRow In orderDetail.Rows
                ...
                purchaseDto.Products.Add(New ProductDto With {
                                         ...
                                         .position = position,
                                         .Price = CType(detailRow("Extended").ToString, Decimal) / CType(detailRow("Quantity").ToString, Integer),
                                         .Quantity = CType(detailRow("Quantity").ToString, Integer),
                                         .Variant = ""
                                         })
                position += 1
            Next
