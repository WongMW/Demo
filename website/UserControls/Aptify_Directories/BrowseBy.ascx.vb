...
#End Region
        ...
        Private Function GetGridSQL() As String
            ...
            Try
                If m_bCompany Then
                    sSQL = "SELECT c.ID, c.Name, WebSite, " & _
                           ...
                           "FROM " & sDatabase & "..vwCompanies c Inner Join vwSubscriptions s on s.RecipientCompanyID=c.ID  Inner Join Product p  on s.ProductID=p.ID and p.Name ='PRACT' and s.Status='Active'"
                    Select Case Trim$(UCase$(m_sBrowseBy))
                        Case "NAME"
                            sSQL = sSQL & " WHERE c.Name LIKE '" & lstBrowse.SelectedItem.Value & "%'"
                        Case "STATE"
                    ...
                    End Select
                    sSQL = sSQL & " AND DirExclude=0 ORDER BY c.Name"
                Else
