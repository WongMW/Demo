...
#End Region
        ...
        Private Sub DoSearch(ByVal sSearchText As String)
            ...
            Dim strSql As String = Database & "..spGetAllSubsidiaries__c @ParentCompanyId=" & lCompanyID
            Dim dt As DataTable = DataAction.GetDataTable(strSql, IAptifyDataAction.DSLCacheSetting.BypassCache)
            ...
            If m_bCompany Then
                ...
                '----------- Sachin Sathe Updated below code--------------------------
                sSQL = "SELECT vwCompanies.ID, Name, WebSite, " & _
                       ...
                       "INNER JOIN dbo.vwSubscriptions ON dbo.vwCompanies.ID = dbo.vwSubscriptions.SubscriberCompanyID WHERE Name LIKE @SearchText " & _
                       "AND vwCompanies.ID in (" + sb.ToString() + ") AND DirExclude=0 AND (dbo.vwSubscriptions.Product = N'PRACT') ORDER BY Name"
            Else
                'Issue 5102
                'sSQL = "SELECT * FROM " & _
                ...
                sSQL = "SELECT vwPersons.ID, FirstLast, Company, " & _
                       "CASE WHEN MailExclude=1 THEN '' ELSE AddressLine1 END 'AddressLine1', " & _
                       "CASE WHEN MailExclude=1 THEN '' ELSE City END 'City', " & _
                       "CASE WHEN MailExclude=1 THEN '' ELSE State END 'State', " & _
                       "CASE WHEN MailExclude=1 THEN '' ELSE ZipCode END 'ZipCode', " & _
                       ...
                       "INNER JOIN dbo.vwSubscriptions ON dbo.vwPersons.ID = dbo.vwSubscriptions.SubscriberID " & _
                       "WHERE NameWCompany LIKE @SearchText " & _
                       "AND DirExclude=0 AND vwPersons.CompanyID in (" + sb.ToString() + ") AND (dbo.vwSubscriptions.Product = N'PRACT') ORDER BY NameWCompany"
            End If
            '----------------------------End-------------------------------------------
