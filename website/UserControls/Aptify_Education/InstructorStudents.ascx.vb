...
#End Region
        ...
        Private Sub CheckInstructor()
            ...
            End If
        End Sub
        'Private Sub cmbCourse_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbCourse.SelectedIndexChanged
        '    ''LoadStudents()
        '    grdStudents.Rebind()
        'End Sub
        ...
        Protected Sub grdFilteredCourses_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles grdStudents.NeedDataSource
            'grdStudents.DataSource = String.Empty
            LoadStudents()
            ''grdStudents.Rebind()
        End Sub
        ...
        Private Sub LoadStudents()
            ...
            Try
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    '    Dim lnk As HyperLink = CType(row.FindControl("lnkCourse"), HyperLink)
                    '    lnk.NavigateUrl = String.Format(ViewClassPage & "?ClassID={0:F0}", dt.Rows((grdStudents.PageIndex * grdStudents.PageSize) + rowcounter)("ClassID").ToString)
                    '    Dim lbl As Label = CType(row.FindControl("lblStartDate"), Label)
                    '    lbl.Text = String.Format("{0:d}", dt.Rows((grdStudents.PageIndex * grdStudents.PageSize) + rowcounter)("StartDate").ToString)
                    '    Dim hlink As HyperLink = CType(row.FindControl("lnkMail"), HyperLink)
                    '    hlink.NavigateUrl = String.Format("mailto:{0}", dt.Rows((grdStudents.PageIndex * grdStudents.PageSize) + rowcounter)("Email1").ToString)
                    '    rowcounter = rowcounter + 1
                    'Next
                    grdStudents.DataSource = dt
                Else
                    ' Suraj S Issue 4/30/13 14452, remove dt.Rows.Add(dt.NewRow) and other lines because when the grid lhaving no records it should display the no record message
                    grdStudents.DataSource = dt
                End If
