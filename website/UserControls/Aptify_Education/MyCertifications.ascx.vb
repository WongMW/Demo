...
#End Region
        ...
        Protected Overridable Sub LoadGrid()
            ...
            Try
                ...
                'For Each row As GridViewRow In grdMyCertifications.Rows
                '    Dim lnk As HyperLink = CType(row.FindControl("lnkTitle"), HyperLink)
                '    lnk.NavigateUrl = String.Format(ViewCertificationPage & "?CertificationID={0:F0}", dt.Rows((grdMyCertifications.PageIndex * grdMyCertifications.PageSize) + rowcounter)("ID").ToString)
                '    Dim lbl As Label = CType(row.FindControl("lblDateGranted"), Label)
                '    lbl.Text = String.Format("{0:d}", dt.Rows((grdMyCertifications.PageIndex * grdMyCertifications.PageSize) + rowcounter)("DateGranted").ToString)
                '    lbl = CType(row.FindControl("lblExpirationDate"), Label)
                '    lbl.Text = String.Format("{0:d}", dt.Rows((grdMyCertifications.PageIndex * grdMyCertifications.PageSize) + rowcounter)("ExpirationDate").ToString)
                '    'Anil issue 13249
        ...
        Protected Sub grdMyCertifications_PageIndexChanging(ByVal sender As Object, ByVal e As GridPageChangedEventArgs) Handles grdMyCertifications.PageIndexChanged
            ''grdMyCertifications.PageIndex = e.NewPageIndex
            LoadGrid()
        End Sub
        Protected Sub grdMyCertifications_PageIndexChanging(ByVal sender As Object, ByVal e As GridPageSizeChangedEventArgs) Handles grdMyCertifications.PageSizeChanged
            ''grdMyCertifications.PageIndex = e.NewPageIndex
            LoadGrid()
        End Sub
