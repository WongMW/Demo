...
#End Region
        ...
        Public Overridable Sub LoadAttachments(ByVal EntityName As String, ByVal RecordID As Long)
            ViewState("AttachmentEntityName") = EntityName
        ...
        Public Overridable Sub LoadAttachments(ByVal EntityID As Long, ByVal RecordID As Long)
            Try
                ...
                Dim sDBOnjectName As Object = Nothing
                Dim sEntityName As String = String.Empty
                sDBOnjectName=CheckUserPermissionToAttachment(sDBOnjectName,sEntityName,EntityID)
                Me.SetProperties()

                Dim sSQL As String, dt As Data.DataTable
                Dim sBaseURL As String
                sBaseURL = "httpS://" & Request.Url.Host & Request.ApplicationPath
                ...
                sSQL = "SELECT ID,Name,Description,BlobSize * 0.0009765625 FileSize ,DateCreated,DateUpdated,'' EncryptedID,'' EncryptedURL FROM " &
                        Database & "..vwAttachmentsWithoutBLOB " &
                ...
                If Not dt Is Nothing Then
                    For Each dr As DataRow In dt.Rows
                        ...
                        dr("EncryptedURL") = "javascript:_do_window_open('" & sBaseURL &
                            "/Handlers/AptifyAttachmentHandler.ashx?AttachmentID=" &
                        dr("EncryptedID").ToString & "&" & "Object=" & HttpUtility.UrlEncode(Aptify.Framework.Web.Common.WebCryptography.Encrypt(sDBOnjectName.ToString)) & "');"
                    Next
            ...
            End Try
        End Sub
         Protected Overridable Function CheckUserPermissionToAttachment(ByRef sDBOnjectName As Object, ByRef sEntityName As String, ByVal EntityID As Long) As Object
            Try
                 If Not ViewState("AttachmentEntityName") Is Nothing Then
                    If ViewState("AttachmentEntityName").ToString.ToLower() = "classes" Then
                        sDBOnjectName = "spCheckUserPermissionforClassAttachments"
                    ElseIf ViewState("AttachmentEntityName").ToString.ToLower() = "committee terms" Then
                        sDBOnjectName = "spCheckUserPermissionforCommitteeTermsAttachment"
                    End If
                Else
                    sEntityName = Me.AptifyApplication.GetEntityName(EntityID)
                    If sEntityName.ToLower() = "discussion forum messages" Then
                        sDBOnjectName = "spCheckUserPermissionforDiscussionForumMessagesAttachment"
                    End If
                End If
                Return sDBOnjectName
            Catch ex As Exception
                Aptify.Framework.ExceptionManagement.ExceptionManager.Publish(ex)
                Return string.Empty
            End Try
        ...
        Protected Sub grdAttachments_RowCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles grdAttachments.ItemCommand
            ...
            End If
        End Sub
        Protected Sub grdAttachments_NeedDataSource(sender As Object, e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles grdAttachments.NeedDataSource
            Me.LoadAttachments(Me.EntityID, Me.RecordID)
        End Sub
