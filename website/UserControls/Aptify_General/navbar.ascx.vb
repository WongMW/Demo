...
Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '  End Sub
        Protected Sub RadMenu1_click(sender As Object, e As Telerik.Web.UI.RadMenuEventArgs)
            If (Request.Url.PathAndQuery.ToString().Contains(".")) Then
                Dim currentItem As RadMenuItem = RadMenu1.FindItemByUrl(Request.Url.PathAndQuery.ToString().Remove(Request.Url.PathAndQuery.ToString().IndexOf(".")))
                ...
                End If
            Else
                Dim currentItemcms As RadMenuItem = RadMenu1.FindItemByUrl(Request.Url.PathAndQuery.ToString())
                If currentItemcms IsNot Nothing Then
                    'Select the current item and his parents
                    currentItemcms.HighlightPath()
                    'Update the title of the
                    ''PageTitleLiteral.Text = currentItemcms.Text
                    'Populate the breadcrumb
                    'DataBindBreadCrumbSiteMap(currentItemcms)
                Else
                    RadMenu1.Items(0).HighlightPath()
                End If
            End If
