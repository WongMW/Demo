...
Namespace Aptify.Framework.Web.eBusiness.ProductCatalog
    Partial Class ShippingControl
        ...
        Protected Const ATTRIBUTE_CONTORL_Refresh As String = "CheckOutControl"
...
#Region "ShippingControl Specific Properties"
 ...
 Public Overridable Property RedirectPage() As String
            Get
                If Not ViewState(ATTRIBUTE_CONTORL_Refresh) Is Nothing Then
                    Return CStr(ViewState(ATTRIBUTE_CONTORL_Refresh))
                Else
                    Return String.Empty
                End If
            End Get
            Set(ByVal value As String)
                ViewState(ATTRIBUTE_CONTORL_Refresh) = Me.FixLinkForVirtualPath(value)
            End Set
        End Property
#End Region
        '<%--Nalini Issue#12578--%>
        Protected Overrides Sub SetProperties()
            ...
            If String.IsNullOrEmpty(ShippingChangeAddressPage) Then
                ...
                Else
                    ...
                    If NameAddressBlock.AddressLine1.Trim <> "" Then
                        Me.lnkChangeAddress.PostBackUrl = ShippingChangeAddressPage & "?Type=Shipping"
                    Else
                        ''Added By Pradip 2016-10-26 For to Add Home Address To Person Which is not having any Address
                        Me.lnkChangeAddress.PostBackUrl = ShippingChangeAddressPage & "?Type=Shipping&IsHome=Y"
                    End If
                End If
            Else
                ' Me.lnkChangeAddress.PostBackUrl = ShippingChangeAddressPage & "?Type=Shipping"
          ...
          If String.IsNullOrEmpty(RedirectPage) Then
                RedirectPage = Me.GetLinkValueFromXML(ATTRIBUTE_CONTORL_Refresh)
            End If
        End Sub
