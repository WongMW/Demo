...
Namespace Aptify.Framework.Web.eBusiness.Generated
   ...
   ''' <summary>
   ''' Generated ASP.NET User Control for the Meeting Rooms entity.
   ''' Description: Tracks an inventory of meeting rooms for multiple venues that can be used across any number of meetings over time.
   ''' </summary>
   ''' <remarks></remarks>
   Partial Class MeetingRoomsClass
       Inherits Aptify.Framework.Web.eBusiness.BaseUserControlAdvanced

       Protected Const ATTRIBUTE_CONTORL_DEFAULT_NAME As String = "Meeting RoomsPage"
       ...
       Protected Overridable Sub LoadRecord()
           Try
               If Me.SetControlRecordIDFromParam() Then
                   LoadDataFromGE(Me.AptifyApplication.GetEntityObject("Meeting Rooms", ControlRecordID))
               Else
                   LoadDataFromGE(Me.AptifyApplication.GetEntityObject("Meeting Rooms", -1))
               End If
       ...
       Protected Overridable Sub SaveRecord(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
           ...
           Try
               If Me.ControlRecordID > 0 Then
                   oGE = AptifyApplication.GetEntityObject("Meeting Rooms", Me.ControlRecordID)
               Else
                   oGE = AptifyApplication.GetEntityObject("Meeting Rooms", -1)
               End If
