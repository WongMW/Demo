Type.registerNamespace("SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer");

SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer.TestimonialsWidgetDesigner = function (element) {
    ...
    this._itemsType = "Telerik.Sitefinity.DynamicTypes.Model.Testimonials.Testimonial";
    this._serviceUrl = "/Sitefinity/Services/DynamicModules/Data.svc/?itemType=Telerik.Sitefinity.DynamicTypes.Model.Testimonials.Testimonial";

    this._selectedItems = kendo.observable({ items: [] });

    this._thumbnailSelect = "img.tmb";

    this._dataItemsArray = [];

    /* Calls the base constructor */
    SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer.TestimonialsWidgetDesigner.initializeBase(this, [element]);
}

SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer.TestimonialsWidgetDesigner.prototype = {
    /* --------------------------------- set up and tear down --------------------------------- */
    initialize: function () {
        /* Here you can attach to events or do other initialization */
        SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer.TestimonialsWidgetDesigner.callBaseMethod(this, 'initialize');
        ...
        var items = this._selectedItems.items;

        kendo.bind($(this.get_selectedItemsList()), this._selectedItems);
    },
    dispose: function () {
        /* this is the place to unbind/dispose the event handlers created in the initialize method */
        SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer.TestimonialsWidgetDesigner.callBaseMethod(this, 'dispose');
    ...
    refreshUI: function () {
        ...
        var value = controlData.SelectedTestimonialIds;
        if (value != null && value != "") {
            ...
            for (var i = 0; i < dataItems.length; i++) {
                ...
                filterExpression = filterExpression + 'Id == ' + item.toString();
            }
            var data = {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    for (var i = 0; i < result.Items.length; i++) {
                        var newItem = result.Items[i];

                        self._selectedItems.items.push(newItem);
                        self._dataItemsArray.push(newItem);
                    }
                }
            });
        }
    ...
    applyChanges: function () {
        ...
        for (var i = 0; i < items.items.length; i++) {
            var item = items.items[i].Id;
            data.push(item);
        }

        controlData.SelectedTestimonialIds = JSON.stringify(data);
    },
    ...
    _selectButtonClicked: function (sender, args) {
        ...
        this._selectDialog.dialog().parent().css("min-width", "100%");
        this._selectDialog.dialog().parent().css("left", "0");
        this._selectDialog.dialog().parent().css("top", "-150px");
    ...
    _DoneSelecting: function (sender, args) {
        ...
        if (selectedItems != null && selectedItems.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                var item = data[i];
                this._selectedItems.items.push(item);
            }
        }
        this._selectDialog.dialog("close");
        jQuery("#designerLayoutRoot").show();
...
SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer.TestimonialsWidgetDesigner.registerClass('SitefinityWebApp.UserControls.SoftwareDesign_Aptify.LayoutWidgets.TestimonialsWidget.Designer.TestimonialsWidgetDesigner', Telerik.Sitefinity.Web.UI.ControlDesign.ControlDesignerBase);
